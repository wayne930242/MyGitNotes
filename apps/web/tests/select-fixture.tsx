...
function Fixture() {
  ...
  const options = [{ value: '', label: 'No status' }, { value: 'done', label: translated ? '完成' : 'Completed' }, { value: 'working', label: 'Working' }, { value: 'waiting', label: 'Waiting', disabled: true }, { value: 'review', label: 'Review' }];
  return (
    <main style={{ padding: 24, display: 'grid', gap: 16 }}>
      ...
      <div onClick={() => setOpens(n => n + 1)}>
        <NoteStatusSelect label='Inline status' status={value} statuses={['done', 'working', 'review']} readOnly={false} onChange={setValue} />
      </div>
