...
import { DEFAULT_NOTE_QUERY, type NoteListItem, type NoteQuery, type NotebookFacets } from '@mygitnotes/core/note-query';
...
const row = (path: string, extra: Partial<NoteListItem> = {}): NoteListItem => ({
  id: path, path, notebookId: 'life', title: path, tags: [], metadata: {}, ...extra,
});
const note = (path: string, extra: Partial<NoteItem> = {}): NoteItem => ({
  id: path, path, notebookId: 'life', title: path, tags: [], metadata: {}, content: '', ...extra,
});
const query = (extra: Partial<NoteQuery> = {}): NoteQuery => ({ ...DEFAULT_NOTE_QUERY, notebookId: 'life', ...extra });
const drafts = (...entries: { note: NoteItem; base: NoteItem | null }[]): WorkingNotes =>
  Object.fromEntries(entries.map(entry => [entry.note.path, entry]));
...
describe('draft rows over a loaded page', () => {
  ...
  it('lists a created draft, and a draft the edit moved into this query, as uncommitted', () => {
    ...
    const result = overlayDraftRows([], query({ status: 'done' }), drafts(
      { note: created, base: null },
      { note: moved, base: note('notes/life/b.md', { status: 'inbox' }) },
    ));
...
describe('draft facets', () => {
  const facets = (): Record<string, NotebookFacets> => ({
    life: { total: 2, hidden: 0, statuses: { inbox: 2 }, tags: { work: 1 }, directories: { 'notes/life': 2 } },
  });

  it('moves a draft between status counts without changing the total', () => {
    const result = overlayDraftFacets(facets(), drafts({
      note: note('notes/life/a.md', { status: 'done' }),
      base: note('notes/life/a.md', { status: 'inbox' }),
    }), false);
  ...
  it('counts a created draft and its tags and directory', () => {
    const result = overlayDraftFacets(facets(), drafts({
      note: note('notes/life/deep/new.md', { status: 'inbox', tags: ['work', 'new'] }),
      base: null,
    }), false);
    expect(result.life).toMatchObject({
      total: 3, statuses: { inbox: 3 }, tags: { work: 2, new: 1 }, directories: { 'notes/life': 2, 'notes/life/deep': 1 },
    });
  });

  it('counts a draft hidden by its status as hidden only', () => {
    const result = overlayDraftFacets(facets(), drafts({
      note: note('notes/life/a.md', { status: 'archived', metadata: { hiden: true } }),
      base: note('notes/life/a.md', { status: 'inbox' }),
    }), false);
...
describe('draft paths, lookups and graph', () => {
  it('drops a path the draft no longer matches and adds one it now matches', () => {
    const staged = drafts(
      { note: note('notes/life/a.md', { status: 'done' }), base: note('notes/life/a.md', { status: 'inbox' }) },
      { note: note('notes/life/new.md', { status: 'inbox' }), base: null },
    );
  ...
  it('replaces a drafted note\'s links and adds a node for a note the server has never seen', () => {
    ...
    const result = overlayGraphDrafts(graph, [
      { path: 'notes/life/a.md', notebookId: 'life', title: 'A', tags: [], content: '[b](../life/b.md)' },
      { path: 'notes/life/b.md', notebookId: 'life', title: 'B', tags: [], content: '' },
    ]);
