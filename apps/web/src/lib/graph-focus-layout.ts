...
export function expandGraphFocus(data: GraphData, focusId: string | undefined, neighbors: Set<string>, zoom: number): GraphData {
  ...
  const related = (data.nodes as PositionedNode[])
    .filter((node): node is PositionedNode & { x: number; y: number } => neighbors.has(node.id) && node.x !== undefined && node.y !== undefined)
    .map(node => {
      const textWidth = Array.from(node.title).reduce((width, char) => width + (/[^\x00-\xff]/.test(char) ? 12 : 7), 0);
      return { id: node.id, x: node.x * scale, y: node.y * scale, ox: node.x * scale, oy: node.y * scale,
        width: Math.max(36, Math.min(168, textWidth) + 20), height: 32 + Math.ceil(textWidth / 168) * 17 };
    });
  for (let iteration = 0; iteration < 80; iteration++) {
    ...
    for (let i = 0; i < related.length; i++) for (let j = i + 1; j < related.length; j++) {
      ...
      if (dx === 0 && dy === 0) { dx = Math.cos(i + j * 2.4); dy = Math.sin(i + j * 2.4); }
      ...
      a.x -= dx * push * aWeight; a.y -= dy * push * aWeight;
      b.x += dx * push * bWeight; b.y += dy * push * bWeight;
    }
    if (!overlap) break;
