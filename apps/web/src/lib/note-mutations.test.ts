...
const note = (extra: Partial<NoteItem> = {}): NoteItem => ({
  id: 'notes/life/a.md', path: 'notes/life/a.md', notebookId: 'life', title: 'A',
  tags: [], metadata: { title: 'A' }, content: '# A\n\nbody\n', status: 'inbox', ...extra,
});
...
it('reads the note in full before writing a status from a list row', async () => {
  ...
  expect(change).toEqual({
    path: 'notes/life/a.md', notebookId: 'life', content: '# A\n\nbody\n',
    metadata: { title: 'A', status: 'done' },
  });
});

it('archives and unarchives through the same metadata rules as the editor', async () => {
  expect((await noteStatusChange(async () => note(), { path: 'notes/life/a.md' }, 'archived')).metadata)
    .toMatchObject({ status: 'archived', hiden: true });
  expect((await noteStatusChange(async () => note({ status: 'archived', metadata: { hiden: true } }), { path: 'notes/life/a.md' }, 'inbox')).metadata)
    .toMatchObject({ status: 'inbox', hiden: false });
});
...
it('writes nothing when the note cannot be read', async () => {
  ...
  await expect(noteStatusChange(read, { path: 'notes/life/a.md' }, 'done'))
