...
interface Snapshot<T> { page: T; revision: string; writable: boolean; path: string }
interface WorkspaceDocumentDraft<T> { page: T; base: T; revision: string; legacy?: boolean }
...
export function useWorkspaceDocument<T>(client: WorkspaceDocumentClient<T>, scope: string, onSaved: () => void, remote = false, enabled = true, config: ScreenNotebookConfig | null = null) {
  ...
  const load = useCallback(async (discard = false) => {
    ...
    try {
      ...
      const record: Snapshot<T> = await response.json(); record.page = document.schema.parse(record.page);
  ...
  const change = (next: T) => {
    ...
    const result = document.schema.safeParse(next); if (!result.success) { setError(t(messages.limit)); return; }
    ...
    catch { setError(t(messages.draft)); }
  };
