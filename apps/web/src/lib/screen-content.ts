...
export function screenRowItems(row: ScreenRow, notes: NoteListItem[], assets: (AssetItem & { notebookId: string; })[], notebooks: NotebookConfig[] = []): ScreenItem[] {
  ...
  return [...selectedNotes.sort((a, b) => a.title.localeCompare(b.title)).map(note => ({ id: `dynamic:${row.id}:${note.path}`, kind: 'note' as const, notebookId: note.notebookId, path: note.path })), ...selectedAssets.sort((a, b) => a.name.localeCompare(b.name)).map(asset => ({ id: `dynamic:${row.id}:${asset.path}`, kind: 'asset' as const, notebookId: asset.notebookId, path: asset.path }))];
}
...
export function studyRowItems(items: ScreenItem[], row: ScreenRow, notes: NoteListItem[], study: StudyWorkspace, now = new Date()): ScreenItem[] {
  ...
  }).filter(entry => (!row.study!.status || entry.note?.status === row.study!.status) && matchesStudyFilter(entry.study, row.study!.filter, now));
