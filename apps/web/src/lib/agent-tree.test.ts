...
const notebooks = [{ id: 'a', title: 'A', root: 'notes/a/' }, { id: 'ab', title: 'AB', root: 'notes/ab' }, { id: 'nested', title: 'Nested', root: 'notes/a/nested' }];
const paths = ['AGENTS.md', 'notes/AGENTS.md', '.agents/skills/review/SKILL.md', '.agents/skills/review/references/check.md', 'notes/a/AGENTS.md', 'notes/ab/AGENTS.md', 'notes/a/nested/AGENTS.md', 'docs/agent/product/index.md'];
const resources: AgentResource[] = paths.map(path => ({ path, name: path, scope: path.startsWith('docs/') ? 'product' : 'workspace' }));
...
describe('Agent document navigation', () => {
  it('keeps shared and product documents while filtering exact notebook boundaries', () => {
    ...
    expect(groupAgentResources(resources, notebooks, 'ab').notebook.map(r => r.path)).toEqual(['notes/ab/AGENTS.md']);
    expect(groupAgentResources(resources, notebooks, 'nested').notebook.map(r => r.path)).toEqual(['notes/a/nested/AGENTS.md']);
  });
  ...
  it('keeps root skills available across notebooks with native settings before docs', () => {
    const files: AgentResource[] = [{ path: '.codex/skills/legacy/SKILL.md', name: 'Legacy', scope: 'workspace' }, { path: '.agents/skills/review/agents/openai.yaml', name: 'Interface', scope: 'workspace' }, { path: '.agents/docs/setup.md', name: 'Setup', scope: 'workspace' }, { path: 'docs/agent/product/skills/dev/SKILL.md', name: 'Product', scope: 'product' }];
  ...
  it('keeps Claude and Antigravity skills in the shared skills section', () => {
    const files: AgentResource[] = ['.claude/skills/review/SKILL.md', '.agent/skills/review/SKILL.md', '.agents/skills/format-tests.md', 'CLAUDE.md', 'GEMINI.md'].map(path => ({ path, name: path, scope: 'workspace' }));
