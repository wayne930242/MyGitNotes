...
export function arrangeGraphLayout(layout: GraphLayout, { compact = false }: { compact?: boolean; } = {}): GraphLayout {
  ...
  if (compact && nodes.length) {
    ...
    for (const node of nodes) {
      if (!node.pinned) {
        node.x = center.x + (node.x - center.x) * .88;
        node.y = center.y + (node.y - center.y) * .88;
      }
    }
  ...
  for (let pass = 0; pass < 120; pass++) {
    ...
    for (let i = 0; i < bounds.length; i++) {
      for (let j = i + 1; j < bounds.length && bounds[j].left < bounds[i].right; j++) {
        ...
        a.x -= dx * (scale + .001) * shareA;
        a.y -= dy * (scale + .001) * shareA;
        b.x += dx * (scale + .001) * shareB;
        b.y += dy * (scale + .001) * shareB;
        moved = true;
      }
    }
    if (!moved) break;
