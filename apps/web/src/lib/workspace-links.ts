export type WorkspaceLink = { kind: 'external'; url: string } | { kind: 'anchor'; anchor: string }
  | { kind: 'path'; path: string; anchor: string } | { kind: 'route'; url: string } | { kind: 'asset-hash'; hash: string };
...
export function resolveWorkspaceHref(value: string, sourcePath: string): WorkspaceLink | null {
  ...
  try {
    ...
    if (/^\/(?:notebooks\/|notes(?:[?#]|$)|assets(?:[?#]|$)|screen(?:[?#]|$))/.test(href)) return { kind: 'route', url: href };
