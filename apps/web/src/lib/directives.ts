...
export function parseDirectiveTitle(
...
export const HANDOUT_VARIANTS: Record<string, string> = {
  report: '文件',
  newspaper: '剪報',
  letter: '信件',
  journal: '日誌',
...
const COC_CHARACTERISTICS = [
  { key: 'str', code: 'STR', label: '力量' },
  { key: 'con', code: 'CON', label: '體質' },
  { key: 'siz', code: 'SIZ', label: '體型' },
  { key: 'dex', code: 'DEX', label: '敏捷' },
  { key: 'int', code: 'INT', label: '智力' },
  { key: 'app', code: 'APP', label: '外貌' },
  { key: 'pow', code: 'POW', label: '意志' },
  { key: 'edu', code: 'EDU', label: '教育' },
  { key: 'san', code: 'SAN', label: '理智' },
  { key: 'hp', code: 'HP', label: '耐久' },
];

const COC_DERIVED = [
  { key: 'db', code: 'DB', label: '傷害加值' },
  { key: 'build', code: 'BUILD', label: '體格' },
  { key: 'move', code: 'MOV', label: '移動率' },
];
...
export function transformMdxComponents(content: string): string {
  ...
  let result = content.replace(
    /<(?:YouTubeEmbed|YouTube)\s+([^>]*?)(?:\/>|>\s*<\/(?:YouTubeEmbed|YouTube)>)/g,
    (_, attrString: string) => {
  ...
  result = result.replace(
    /<ProtectedContent\b([^>]*)>([\s\S]*?)<\/ProtectedContent>/g,
    (_, attrString: string, body: string) => {
      const attrs = parseDirectiveAttributes(attrString);
  ...
  result = result.replace(
    /<(?:Tag|Badge)\b([^>]*)>([\s\S]*?)<\/(?:Tag|Badge)>/g,
    (_, _attrString: string, body: string) => {
  ...
  result = result.replace(
    /<([A-Z][a-zA-Z0-9_-]*)\s*([^>]*?)\/>/g,
    (_, name: string, attrString: string) => {
      const attrs = parseDirectiveAttributes(attrString);
 ...
 */
export function renderContainerDirective(
  name: string,
  label: string | undefined,
  attrs: Record<string, string>,
  bodyMarkdown: string,
  ...
  if (normName === 'parallel-quote') {
    ...
    const attribution = cite || author || source
      ? `<footer class="parallel-quote-attribution">${author ? `<span data-label="AUTHOR">${escapeHtml(author)}</span>` : ''}${source ? `<cite class="parallel-quote-source" data-label="SOURCE">${escapeHtml(source)}</cite>` : ''}${cite ? `<span class="parallel-quote-bibliography" data-label="CITATION">${escapeHtml(cite)}</span>` : ''}</footer>`
      : '';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (closeMatch && stack.length > 0) {
      ...
      if (stack[stack.length - 1].fenceLength === fenceLength) {
        ...
        const renderedHtml = renderContainerDirective(
          item.name,
          item.label,
          item.attrs,
          transformedInner,
...
export const DIRECTIVE_TEMPLATES: DirectiveTemplate[] = [
  ...
  },
  {
    type: 'optional',
    label: '摺疊細節 (Optional)',
    defaultSnippet: ':::optional[點擊展開詳細內容]\n收合的細節說明...\n:::\n'
  },
  {
    type: 'comment',
    label: '作者眉批 (Comment)',
  ...
  },
  {
    type: 'parallel-quote',
    label: '雙語對照 (Quote)',
    defaultSnippet: ':::parallel-quote{author="哲學家" source="著作名稱" cite="[@citationKey]"}\n:::original\nOriginal quotation text here.\n:::\n:::translation\n在此輸入繁體中文譯文。\n:::\n:::\n'
  },
  {
    type: 'github-repo',
    label: 'GitHub 專案卡片',
    defaultSnippet: ':::github-repo{url="https://github.com/user/repo" title="專案名稱"}\n專案簡介與特色說明\n:::\n'
  },
  {
    type: 'x-post',
    label: 'X (Twitter) 卡片',
    defaultSnippet: ':::x-post{url="https://x.com/user/status/123" title="貼文標題"}\n貼文摘錄或討論重點\n:::\n'
  },
  {
    type: 'reddit-post',
    label: 'Reddit 卡片',
    defaultSnippet: ':::reddit-post{url="https://reddit.com/r/..." title="討論串"}\n討論摘要\n:::\n'
  },
  {
    type: 'grid',
    label: '雙欄網格 (Grid)',
    defaultSnippet: '::::grid{cols="2" gap="1rem"}\n:::cell\n左側內容\n:::\n:::cell\n右側內容\n:::\n::::\n'
  }
];
      ...
      if (stack[stack.length - 1].fenceLength === fenceLength) {
        ...
        if (stack.length === 0) {
          ...
          blocks.push({
            from: item.from,
            to: lineEnd,
            fenceLength: item.fenceLength,
            type: item.type,
            label: item.label,
            attrs: item.attrs,
            headerLine: item.headerLine,
            rawBody: item.bodyLines.join('\n'),
            rawText,
          });
        } else {
