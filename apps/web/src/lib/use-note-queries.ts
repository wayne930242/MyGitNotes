...
import type { NoteAgenda, NoteFacets, NoteListItem, NoteQuery, NoteQueryPage, NotebookFacets } from '@mygitnotes/core/note-query';
 ...
 */
export interface NoteQueryScope { sourceId: string; revision: string; drafts: WorkingNotes }
...
export function useNoteQueryScope(): NoteQueryScope {
  return useSyncExternalStore(listener => { listeners.add(listener); return () => { listeners.delete(listener); }; }, readScope, readScope);
}
...
export function noteLookupOptions(scope: NoteQueryScope, paths: string[], content: boolean) {
  ...
  return {
    queryKey: queryKey(scope, 'lookup', { paths: unique, content }),
    queryFn: () => lookupNotes(unique, { content, revision: revisionOf(scope) }),
    enabled: unique.length > 0 && Boolean(scope.sourceId),
  };
}
...
export function notePageOptions(scope: NoteQueryScope, query: Partial<NoteQuery>, options: { limit?: number; content?: boolean } = {}) {
  ...
  return {
    queryKey: queryKey(scope, 'page', { query: input, limit: options.limit ?? NOTE_PAGE_SIZE, content: Boolean(options.content) }),
    queryFn: () => fetchNoteQuery(input, { limit: options.limit ?? NOTE_PAGE_SIZE, content: options.content, revision: revisionOf(scope) }),
  };
}
...
export function notePathsOptions(scope: NoteQueryScope, query: Partial<NoteQuery>) {
  const input = noteQueryInput(query);
  return {
    queryKey: queryKey(scope, 'paths', input),
    queryFn: () => fetchNotePaths(input, revisionOf(scope)),
...
export interface NoteListOptions { limit?: number; content?: boolean; hide?: string }
...
export function useNoteList(query: Partial<NoteQuery> | null, options: NoteListOptions = {}): NoteListResult {
  ...
  const result = useInfiniteQuery({
    queryKey: queryKey(scope, 'query', { query: input, limit, content }),
    queryFn: ({ pageParam }) => fetchNoteQuery(input!, { revision: revisionOf(scope), cursor: pageParam, limit, content }),
    initialPageParam: undefined as string | undefined,
    getNextPageParam: (page: NoteQueryPage) => page.nextCursor ?? undefined,
    enabled: Boolean(input) && Boolean(scope.sourceId),
  ...
  const overlay = useMemo(
    () => (input ? overlayDraftRows(rows, input, scope.drafts, { hide: options.hide }) : { notes: [], uncommitted: [], removed: 0 }),
    [rows, input, scope.drafts, options.hide],
  );
  const fetchNextPage = result.fetchNextPage, hasNextPage = result.hasNextPage, fetchingNext = result.isFetchingNextPage;
  const loadMore = useCallback(() => { if (hasNextPage && !fetchingNext) void fetchNextPage(); }, [hasNextPage, fetchingNext, fetchNextPage]);
  const serverTotal = (input && result.data?.pages[0]?.total) || 0;
  return {
    notes: overlay.notes,
    uncommitted: overlay.uncommitted,
    total: Math.max(0, serverTotal - (options.hide ? 1 : 0) - overlay.removed + overlay.uncommitted.length),
    loading: Boolean(input) && result.isPending,
    loadingMore: fetchingNext,
...
export function useNotePaths(query: Partial<NoteQuery> | null): { paths: string[]; loading: boolean; error: string } {
  ...
  const paths = useMemo(
    () => (input ? overlayDraftPaths(result.data?.paths ?? [], input, scope.drafts) : []),
    [result.data, input, scope.drafts],
  );
...
export function useNoteFacets(showHidden: boolean): { facets: Record<string, NotebookFacets> | undefined; loading: boolean; error: string } {
  ...
  const result = useQuery({
    queryKey: queryKey(scope, 'facets', { showHidden }),
    queryFn: () => fetchNoteFacets(showHidden, revisionOf(scope)),
    enabled: Boolean(scope.sourceId),
    placeholderData: keepPreviousData,
  });
  const facets = useMemo(
    () => result.data ? overlayDraftFacets((result.data as NoteFacets).notebooks, scope.drafts, showHidden) : undefined,
...
export function useNoteAgenda(notebookId: string, showHidden = false): { agenda: NoteAgenda | undefined; loading: boolean; error: string } {
  const scope = useNoteQueryScope();
  const result = useQuery({
    queryKey: queryKey(scope, 'agenda', { notebookId, showHidden }),
    queryFn: () => fetchNoteAgenda(notebookId, { showHidden, revision: revisionOf(scope) }),
    enabled: Boolean(scope.sourceId) && Boolean(notebookId),
    placeholderData: keepPreviousData,
  });
  const agenda = useMemo(
    () => result.data ? overlayDraftAgenda(result.data, scope.drafts, { notebookId, showHidden }) : undefined,
    [result.data, scope.drafts, notebookId, showHidden],
  );
...
export function useNoteGraph(enabled = true): { graph: NoteGraphData | undefined; loading: boolean; error: string } {
  const scope = useNoteQueryScope();
  const result = useQuery({
    queryKey: queryKey(scope, 'graph', {}),
    queryFn: () => fetchNoteGraph(revisionOf(scope)),
    enabled: enabled && Boolean(scope.sourceId),
    placeholderData: keepPreviousData,
  });
  const graph = useMemo(
    () => (enabled && result.data ? overlayGraphDrafts({ nodes: result.data.nodes, links: result.data.links }, draftGraphNotes(scope.drafts)) : undefined),
    [enabled, result.data, scope.drafts],
  );
