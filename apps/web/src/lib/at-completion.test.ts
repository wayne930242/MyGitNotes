...
describe('getAtCompletionItems', () => {
  ...
  it('adds due-date items and a date picker item on a task line', () => {
    ...
    expect(ids).toEqual(['now', 'today', 'yesterday', 'tomorrow', 'due-today', 'due-tomorrow', 'due-next-monday', 'pick-date']);
  ...
  it('leaves the pick-date item without insert text, as a signal to open a date picker', () => {
    const items = getAtCompletionItems({ onTaskLine: true, now });
    const pickDate = items.find(item => item.id === 'pick-date');
    expect(pickDate?.insertText).toBeUndefined();
  });
});
