...
it('separates expanded cards and the surrounding collapsed nodes in both dimensions', () => {
  const layout: GraphLayout = { nodes: [
    { path: 'a.md', x: -30, y: -20, expanded: true, width: 500, height: 400 },
    { path: 'b.md', x: 30, y: 20, expanded: true },
    { path: 'c.md', x: -10, y: 25 }, { path: 'd.md', x: 35, y: -25 },
  ] };
...
it('keeps manually pinned cards fixed while moving nearby nodes around their full size', () => {
  const result = arrangeGraphLayout({ nodes: [
    { path: 'a.md', x: 0, y: 0, width: 800, height: 600, expanded: true, pinned: true },
    { path: 'b.md', x: 100, y: 50 }, { path: 'c.md', x: -100, y: -50, expanded: true },
  ] });
