...
import { AlertTriangle, Check, ChevronDown, ChevronUp, Code, Code2, Copy, Eye, FileText, LayoutGrid, ListOrdered, PanelRight, RotateCcw, Save, Search, X } from 'lucide-react';
...
export const NoteEditor = forwardRef<NoteEditorHandle, NoteEditorProps>(({ note, frame, active, documentPanel, onClose, onAddToFocus, onSession, onCaret, statuses, metadataFields, readOnly = false, autoSave = true, draftMode = false, remoteBase, conflictReason, onMarkConflict, onSave, onReadRemote, onRestoreFile, isDirty: propIsDirty = false, availableTags = [], branch, draftScope }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    insert(text, at) {
      if (!locked) editorRef.current?.insert(text, at);
    },
  }), [locked]);
  ...
  const panelSections = (
    <>
      ...
      {isAssetPickerOpen && <FileManager notebookId={note.notebookId} writable={false} mode='pick-image' layout='panel' onInsert={locked ? undefined : handleInsertAssetRef} />}
      {isGitPanelOpen && (
        <div className='note-git-panel note-panel-scroll'>
          <dl>
            <div>
              <dt>{t('editor.filePath')}</dt>
              <dd className='font-mono'>{note.path}</dd>
            </div>
            <div>
  ...
  return (
    <div className='note-editor' data-frame={frame}>
      <div className='editor-notices'>
        ...
        {recoveredDraft && !blocked && <CrashRecoveryBanner draft={{ path: note.path, content: recoveredDraft.content, metadata: recoveredDraft.metadata, savedAt: recoveredDraft.savedAt }} onRestore={handleRestoreDraft} onDiscard={handleDiscardDraft} />}
        {saveError && <EditorNotice tone='error'>{t(saveError as TranslationKey, saveErrorParams)}</EditorNotice>}
        {(blocked || showRemoteNotice || showConflictDraftNotice) && (
          <EditorNotice
            actions={
              <>
                ...
                {!blocked && (
                  <button type='button' aria-label={t('editor.dismissNotice')} title={t('editor.dismissNotice')} onClick={dismissNotice} className='ui-icon-button'>
                    <X aria-hidden='true' />
                  </button>
                )}
              </>
          ...
          >
            {showRemoteNotice ? t(remoteNotice as TranslationKey) : t('editor.localChangesPreserved')}
          </EditorNotice>
        )}
      </div>
      {frame === 'compact'
        ? (
          <>
            ...
            <div className='note-compact-bar'>
              ...
              <button type='button' className='ui-icon-button' title={t(copyState === 'copied' ? 'editor.noteCopied' : copyState === 'error' ? 'editor.noteCopyFailed' : 'editor.copyNote')} aria-label={t(copyState === 'copied' ? 'editor.noteCopied' : copyState === 'error' ? 'editor.noteCopyFailed' : 'editor.copyNote')} onClick={copyNote}>{copyState === 'copied' ? <Check size={14} aria-hidden='true' /> : copyState === 'error' ? <AlertTriangle size={14} aria-hidden='true' /> : <Copy size={14} aria-hidden='true' />}</button>
              <span className='note-compact-path' title={note.path}>{note.notebookId}{' · '}{note.path.split('/').pop()}</span>
              <span role='status' className='note-compact-status' data-state={editorState}>
                <span className={`note-compact-dot ${editorState === 'saving' ? 'animate-pulse' : ''}`} aria-hidden='true' />
                {editorStatus}
              </span>
            </div>
          </>
          ...
          <>
            ...
            <div className='note-toolbar relative shrink-0 px-5 py-3.5 border-b border-line flex items-center justify-between gap-4 bg-sidebar/60'>
              {frame === 'zoom' && (
                <div className='note-heading flex items-center gap-3 truncate'>
                  ...
                  <div className='truncate'>
                    <div className='font-serif font-semibold text-fg text-sm truncate'>{String(metadata.title || note.title || t('editor.untitled'))}</div>
              ...
              <div className='note-controls flex items-center gap-2'>
                {!autoSave && !readOnly && (
                  <Button variant='primary' aria-label={t('editor.saveToGitHub')} title={t('editor.saveToGitHub')} disabled={locked || !hasUnsavedChanges} onClick={handleExplicitSave} className='note-save editor-action'>
                    <Save className='editor-mobile-icon w-5 h-5' />
                    <span>{isSaving ? t('editor.saving') : t('editor.saveToGitHub')}</span>
                  </Button>
                )}
                ...
                <button type='button' aria-label={t(copyState === 'copied' ? 'editor.noteCopied' : copyState === 'error' ? 'editor.noteCopyFailed' : 'editor.copyNote')} title={t(copyState === 'copied' ? 'editor.noteCopied' : copyState === 'error' ? 'editor.noteCopyFailed' : 'editor.copyNote')} onClick={copyNote} className='ui-icon-button toolbar-icon-button'>{copyState === 'copied' ? <Check aria-hidden='true' /> : copyState === 'error' ? <AlertTriangle aria-hidden='true' /> : <Copy aria-hidden='true' />}</button>
                {onAddToFocus && (
                ...
                {frame === 'zoom' && (
                  <button
                    ...
                    aria-pressed={Boolean(notePanel)}
                    onClick={() => {
                      if (notePanel) setNotePanel(null);
                      else if (lastNotePanel.current === 'outline') {
                        if (isMarkdown) openOutline();
                        else openFind();
                      } else if (lastNotePanel.current === 'find') openFind();
                      else setNotePanel(lastNotePanel.current);
                    }}
                ...
                {/* Close Button */}
                {onClose && (
                  <button aria-label={t('editor.closeNote')} title={t('editor.closeNote')} onClick={close} className='note-close ui-icon-button toolbar-icon-button'>
                    <X aria-hidden='true' />
                  </button>
                )}
              </div>
            ...
            <div className='note-editor-body'>
              ...
              {frame === 'zoom' ? <aside className='note-document-panel' data-open={Boolean(notePanel)} data-panel={notePanel || undefined} aria-label={t('editor.documentPanel')}>{panelTabs}{panelSections}</aside> : documentPanel?.target && notePanel && createPortal(<section className='note-document-panel' data-open='true' data-panel={notePanel} data-frame='rail' aria-label={t('editor.documentPanel')}>{panelSections}</section>, documentPanel.target)}
            </div>
            <EditorFooter content={content} path={note.path} branch={branch} state={editorState} status={editorStatus} />
            {isEditorLeaderOpen && (
              <div className='note-editor-leader' role='dialog' aria-modal='false' aria-label={t('editor.noteCommands')}>
                ...
                {isMarkdown && (
                  <button type='button' onClick={openOutline}>
                    <kbd>/</kbd>
                    <span>{t('editor.outline')}</span>
                  </button>
                )}
