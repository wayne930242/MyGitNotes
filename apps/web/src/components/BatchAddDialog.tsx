 ...
 */
export const BatchAddDialog: React.FC<{ focus: NoteFocus; target: string; pane: number; notebookId: string; notebookRoot: string; folders: readonly FolderItem[]; onClose: () => void; }> = ({ focus, target, pane, notebookId, notebookRoot, folders, onClose }) => {
  ...
  const folderOptions = useMemo(() => folders.filter(item => item.notebookId === notebookId).map(item => ({ value: `${notebookRoot}/${item.path}`, label: item.title || item.path })).sort((a, b) => a.label.localeCompare(b.label)), [folders, notebookId, notebookRoot]);
  ...
  const submit = (event: React.FormEvent) => {
    ...
    try {
      outcome = focus.addBatch(target, tabs, pane);
    } catch (caught) {
  ...
  return (
    <WorkspaceDialog title={t('focus.batchAdd')} onClose={onClose} className='focus-batch-dialog'>
      <form className='screen-form' onSubmit={submit}>
        {!result && (
          <>
            <label className='focus-batch-folder'>
              {t('focus.batchAddFolder')}
              <Select value={folder} disabled={Boolean(result)} onValueChange={setFolder} options={[{ value: '', label: t('folder.allFolders') }, ...folderOptions]} />
            </label>
            <label className='filter-check'>
              <input type='checkbox' checked={descendants} disabled={Boolean(result)} onChange={event => setDescendants(event.target.checked)} />
            ...
            <fieldset>
              ...
              <label className='filter-tag-mode'>
                {t('filters.tagMode')}
                <Select value={tagMode} disabled={Boolean(result)} onValueChange={value => setTagMode(value as 'any' | 'all')} options={[{ value: 'any', label: t('filters.any') }, { value: 'all', label: t('filters.all') }]} />
              </label>
            ...
            {hasFolder && hasTags && (
              <label className='focus-batch-combine'>
                {t('focus.batchAddCombine')}
                <Select value={combine} disabled={Boolean(result)} onValueChange={value => setCombine(value as 'or' | 'and')} options={[{ value: 'or', label: t('focus.batchAddCombineOr') }, { value: 'and', label: t('focus.batchAddCombineAnd') }]} />
              </label>
            )}
        ...
        {result && (
          <p className='focus-batch-result' role='status'>
