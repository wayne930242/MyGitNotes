...
export function GraphNoteCard({ node, session, editorRef, onSession, onCollapse, onMaximize, onSelect, onMove, onResize, onConnect, onLink, onCaret, selected, maximized, color }: {
  ...
  onLink: (target: { path: string; title: string; }) => void;
  ...
  return (
    <article className={`graph-note-card ${selected ? 'is-selected' : ''}`} style={{ borderColor: color, '--graph-node-color': color } as React.CSSProperties} data-graph-note={node.id} aria-label={node.title}>
      <header
        onPointerDown={event => {
          if (!(event.target as Element).closest('button,select,[role="combobox"],.editor-mode-switch') || (event.target as Element).closest('.graph-note-title')) onMove(event);
        }}
      >
      ...
      <button
        type='button'
        className='graph-connection-port'
        ...
        title={t('graph.connect')}
      />
      {linking && (
        <div className='note-link-picker'>
          <input type='search' autoFocus value={query} aria-label={t('graph.findNote')} onChange={event => setQuery(event.target.value)} />
          {candidates.map(target => (
            <button
              key={target.path}
              onClick={() => {
                onLink({ path: target.path, title: target.title });
                setLinking(false);
              }}
            >
              {target.title}
