...
import { GitCommit, RotateCcw, Plus, Minus, RefreshCw } from 'lucide-react';
...
function Changes({ request, writable, gitStatus, remoteChanges, getPreview, restoreFile, commitFiles, onChanged, onCommitted, onClose }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    setDiff(''); setDiffLoading(false); setDiffError('');
    if (!active || !selected?.available) return;
    if (remoteDiff !== undefined) { setDiff(remoteDiff); return; }
    setDiffLoading(true);
    void fetchFileDiff(active.path, selectionMode ? 'current' : active.side).then(value => { if (!cancelled) setDiff(value); }).catch(error => { if (!cancelled) setDiffError(error.message); }).finally(() => { if (!cancelled) setDiffLoading(false); });
    return () => { cancelled = true; };
  ...
  const restoreMany = async () => {
    ...
    try {
      for (const file of restoreFiles) {
        if (remote) await restoreFile?.(file);
        else { const result = await manageFileChange(file, 'restore'); if (result.backup) backups.push(result.backup); }
        restored++;
      }
    } catch (error) { setError(`${t('changes.restoredCount', { count: restored })} ${(error as Error).message}`); }
    finally {
  ...
  const group = (side: 'working' | 'staged') => <section className="changes-group" aria-label={t(selectionMode ? 'panel.changesFiles' : side === 'working' ? 'changes.unstaged' : remote ? 'changes.included' : 'changes.staged')}>
    <h4>{t(selectionMode ? 'panel.changesFiles' : side === 'working' ? 'changes.unstaged' : remote ? 'changes.included' : 'changes.staged')} <small>{changes.filter(file => selectionMode || (side === 'working' ? file.unstaged : file.staged)).length}</small></h4>
    {changes.filter(file => selectionMode || (side === 'working' ? file.unstaged : file.staged)).map(file => <div className="changes-row" key={file.path} data-change-path={file.path} data-side={side}>
      ...
      <Button type="button" size="icon" aria-label={`${t('common.restore')} ${file.path}`} title={t(file.tracked ? 'common.restore' : 'changes.discardNew')} disabled={busy || !writable || !file.available} onClick={() => void act(file, 'restore')}><RotateCcw /></Button>
    </div>)}
  </section>;
  return <WorkspaceDialog title={t('changes.title')} className="changes-dialog" onClose={() => { if (!busy) onClose(); }}>
      <header><div><small>{t('commit.branch', { branch: gitStatus?.branch || '' })}</small></div>
        <Button size="icon" aria-label={t('changes.refresh')} disabled={busy} onClick={() => { setError(''); void refresh().catch(error => setError(error.message)); }}><RefreshCw /></Button>
      </header>
      ...
      {restoreFiles && <EditorNotice actions={<>
        <Button disabled={busy} onClick={() => setRestoreFiles(undefined)}>{t('common.cancel')}</Button>
        <Button variant="danger" disabled={busy || !restoreFiles.length} onClick={() => void restoreMany()}>{t('changes.confirm')}</Button>
      ...
      {confirm && <div className="changes-confirm" role="alert"><p>{t(confirm.tracked ? 'changes.confirmRestore' : 'changes.confirmDiscard', { path: confirm.path })}</p>
        ...
        <Button variant="danger" aria-label={t('changes.confirm')} disabled={busy} onClick={() => void act(confirm, 'restore', true)}>{t('changes.confirm')}</Button></div>}
      ...
      <footer><input className="ui-control" aria-label={t('commit.message')} placeholder={t('commit.placeholder')} value={message} disabled={busy} onChange={event => setMessage(event.target.value)} />
        ...
        <Button variant="primary" disabled={busy || !writable || !staged.length || !message.trim() || staged.some(file => !file.available)} onClick={() => void commit()}><GitCommit />{t(busy ? 'commit.committing' : selectionMode ? 'changes.commitSelected' : remote ? 'commit.commitToGithub' : 'commit.commitAndSave', { count: staged.length })}</Button></footer>
  </WorkspaceDialog>;
}
