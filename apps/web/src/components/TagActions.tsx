...
export const TagActions: React.FC<TagActionsProps> = ({ tag, allTags, onPreviewUsage, onRename, onMerge, onDelete, children }) => {
  ...
  useEffect(() => {
    setPortal(triggerRef.current?.closest('dialog') || undefined);
  }, [mode]);
  useEffect(() => {
    if (mode !== 'closed') formRef.current?.scrollIntoView({ block: 'nearest' });
  }, [mode]);
  useEffect(() => {
    if (mode === 'closed' && returnFocus) {
      triggerRef.current?.focus();
      /* eslint-disable react/set-state-in-effect -- Restore focus after the dialog closes, then consume the request; clearing it in the close handler would run before the DOM transition. */
      setReturnFocus(false);
      /* eslint-enable react/set-state-in-effect */
    }
  }, [mode, returnFocus]);
  const menuId = useId();
  const { onTouchStart, onTouchMove, onTouchEnd, onTouchCancel, onContextMenu } = useLongPress(() => {
  ...
  const isNewTarget = mode === 'merge' && trimmedTarget.length > 0 && !sameName && !allTags.some(candidate => candidate.toLocaleLowerCase(language) === trimmedTarget.toLocaleLowerCase(language));
  ...
  if (mode !== 'closed') {
    return (
      <div className='sidebar-tag-action-form' ref={formRef} onClick={stop} role='group' aria-label={mode === 'rename' ? t('sidebar.tagRenameTitle', { tag }) : t('sidebar.tagMergeTitle', { tag })}>
        <span className='sidebar-tag-action-target'>{mode === 'rename' ? t('sidebar.tagRenameTitle', { tag }) : t('sidebar.tagMergeTitle', { tag })}</span>
        ...
        <div className='sidebar-tag-action-buttons'>
          ...
          <Button variant='primary' disabled={busy || previewLoading || !trimmedTarget || sameName || count === null || count === 0} onClick={() => void submit()}>{mode === 'rename' ? t('sidebar.tagConfirmRename', { count: count ?? 0 }) : t('sidebar.tagConfirmMerge', { count: count ?? 0 })}</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <DropdownMenu.Root open={menuOpen} onOpenChange={handleOpenChange}>
      ...
      <span className='sidebar-tag-actions-trigger' onClick={stop} data-open={menuOpen || undefined}>
        <DropdownMenu.Trigger ref={triggerRef} className='ui-icon-button' aria-label={t('sidebar.tagManage', { tag })} title={t('sidebar.tagManage', { tag })}>
        ...
        <DropdownMenu.Content
          ...
          onCloseAutoFocus={event => {
            ...
            if (openingFormRef.current) {
              ...
              event.preventDefault();
              chipRef.current?.querySelector('button')?.focus();
            }
          }}
        >
          ...
          <DropdownMenu.Item
            ...
            onSelect={event => {
              event.preventDefault();
              if (!busy) void handleDeleteClick();
            }}
          >
            {deleteArmed && count !== null && count > 0 ? t('sidebar.tagConfirmDelete', { count }) : t('sidebar.tagDelete')}
          </DropdownMenu.Item>
          {deleteArmed && count !== null && count > 0 && <p id={`${menuId}-status`} role='status' className='sidebar-tag-action-error'>{t('sidebar.tagConfirmDeleteAgain')}</p>}
