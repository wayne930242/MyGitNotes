...
export function AssetLibrary({ assets, onUploadAsset, onDeleteAsset, onMoveAsset, onInsert }: AssetLibraryProps) {
  ...
  return <div className="asset-library flex flex-col gap-4 theme-text min-h-0 min-w-0">
    ...
    <div className="flex flex-wrap gap-2">{folders.filter(f => f && f !== directory).map(folder => <button key={folder} className="ui-button" disabled={busy} onClick={() => { setDirectory(folder); setSelectedPath(''); }}>{folder}</button>)}</div>
    {visible.length === 0 ? <p className="text-sm theme-muted py-10 text-center">{t('assets.emptyFolder')}</p> : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 overflow-y-auto">
