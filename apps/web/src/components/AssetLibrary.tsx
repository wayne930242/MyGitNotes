...
export function AssetLibrary({ assets, initialAssetPath, initialDirectory, onUploadAsset, onDeleteAsset, onMoveAsset, onInsert, renderHeader, renderSidebar, onBusyChange }: AssetLibraryProps) {
  ...
  const upload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await run(async () => {
      const asset = await onUploadAsset(file, directory);
      setSelectedPath(asset.path);
    });
  };
  const selectDirectory = (value: string) => {
    if (!busy) {
      setDirectory(value);
      setSelectedPath('');
    }
          ...
          <div className='asset-library flex flex-col gap-4 theme-text min-h-0 min-w-0'>
            <div className='flex flex-wrap items-end gap-3'>
              <label className='text-xs flex-1 min-w-40'>
                ...
                <input
                  ...
                  value={directory}
                  disabled={busy}
                  placeholder={t('assets.rootPlaceholder')}
                  onChange={e => {
              ...
              <label aria-disabled={!onUploadAsset || busy} className={`${button} ${onUploadAsset && !busy ? 'cursor-pointer' : 'opacity-40 cursor-not-allowed'}`}>
                ...
                {busy ? t('assets.working') : t('assets.uploadAsset')}
            ...
            {!renderSidebar && (
              <div className='flex flex-wrap gap-2'>
                {folders.filter(f => f && f !== directory).map(folder => (
                  <button
                    key={folder}
                    className='ui-button'
                    disabled={busy}
            ...
            {visible.length === 0 ? <p className='text-sm theme-muted py-10 text-center'>{t('assets.emptyFolder')}</p> : (
              <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3'>
                {visible.map(asset => (
                  <button
                    key={asset.path}
                    aria-label={`Select ${asset.name}`}
                    aria-pressed={selectedPath === asset.path}
                    disabled={busy}
                    onClick={() => {
                      setSelectedPath(asset.path);
                      setConfirmDelete(false);
                    }}
                    className='asset-tile relative border rounded-xl overflow-hidden text-left transition hover:shadow-sm'
                    style={{ backgroundColor: 'var(--color-surface)', borderColor: selectedPath === asset.path ? 'var(--color-primary)' : 'var(--color-border)', boxShadow: selectedPath === asset.path ? '0 0 0 1px var(--color-primary)' : undefined }}
                  >
                    ...
                    <div className='p-2.5'>
                      ...
                      <div className='text-[10px] theme-muted mt-1'>{(asset.size / 1024).toFixed(1)}{' KB'}</div>
                    </div>
            ...
            {selected && (
              <div className='border-t theme-border pt-3 flex flex-wrap items-center gap-2'>
                ...
                >
                  {copiedRef ? t('assets.copied') : t('assets.copyReference')}
                </button>
                <button
                  className={`${button} ui-button-danger`}
                  disabled={!selected || !onDeleteAsset || busy}
                  onClick={() => {
            ...
            {onMoveAsset && selected && (
              <div className='flex gap-2 items-center'>
                ...
                <button
                  className={button}
                  disabled={!selected || busy || destination === (selected.directory || '')}
                  onClick={() =>
                    selected && void run(async () => {
                      const moved = await onMoveAsset(selected, destination);
            ...
            {preview && (
              <div role='dialog' aria-label='Asset preview' aria-modal='true' className='viewport-overlay fixed inset-0 z-[70] bg-scrim/80 flex items-center justify-center p-6' onClick={() => setPreview(null)}>
                <div className='ui-dialog overflow-hidden min-w-0 max-w-4xl max-h-[85dvh] shadow-xl' onClick={e => e.stopPropagation()}>
                  <div className='p-3 flex items-center gap-4 border-b theme-border'>
                    <ImageIcon className='w-4 h-4' />
                    <span className='text-sm mr-auto min-w-0 truncate'>{preview.name}</span>
                    <a href={preview.rawUrl} target='_blank' rel='noopener noreferrer' className='text-xs underline shrink-0'>{t('assets.openOriginal')}</a>
                    <button aria-label='Close asset preview' onClick={() => setPreview(null)} className='ui-icon-button'>
                      <X className='w-5 h-5' />
                    </button>
                  </div>
