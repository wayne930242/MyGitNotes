  ...
  return <div className="asset-library flex flex-col gap-4 text-slate-800 dark:text-slate-200 min-h-0 min-w-0">
    <div className="flex flex-wrap items-end gap-3">
      <label className="text-xs flex-1 min-w-40"><span className="flex gap-1 items-center mb-1"><Folder className="w-3.5 h-3.5" />{t('assets.folder')}</span>
        <input aria-label="Asset folder" list={folderList} value={directory} disabled={busy} placeholder={t('assets.rootPlaceholder')} onChange={e => { setDirectory(e.target.value); setSelectedPath(''); }} className="w-full px-3 py-2 rounded-lg border bg-transparent dark:border-slate-700" />
      </label>
      ...
      <label className={`${button} flex items-center gap-1.5 ${onUploadAsset && !busy ? 'cursor-pointer' : 'opacity-40 cursor-not-allowed'}`}>
        ...
        <input aria-label={t('assets.uploadAsset')} type="file" disabled={!onUploadAsset || busy} onChange={upload} className="hidden" />
      </label>
    </div>
    ...
    {visible.length === 0 ? <p className="text-sm text-slate-400 py-10 text-center">{t('assets.emptyFolder')}</p> : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 overflow-y-auto">
    ...
    <div className="border-t pt-3 flex flex-wrap items-center gap-2 dark:border-slate-700">
      ...
      {onInsert && <button className={`${button} text-white hover:opacity-90`} style={{ backgroundColor:'var(--color-primary)' }} disabled={!selected || busy} onClick={() => selected && onInsert(selected)}>{t('assets.insert')}</button>}
    </div>
    {onMoveAsset && <div className="flex gap-2 items-center">
      <input aria-label="Move asset to folder" list={folderList} placeholder={t('assets.destinationPlaceholder')} value={destination} disabled={!selected || busy} onChange={e => setDestination(e.target.value)} className="min-w-0 flex-1 px-3 py-2 rounded-lg border text-xs bg-transparent dark:border-slate-700" />
    ...
    {preview && <div role="dialog" aria-label="Asset preview" aria-modal="true" className="viewport-overlay fixed inset-0 z-[70] bg-slate-950/80 flex items-center justify-center p-6" onClick={() => setPreview(null)}>
      <div className="bg-white dark:bg-slate-900 rounded-xl overflow-hidden min-w-0 max-w-4xl max-h-[85dvh] shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="p-3 flex items-center gap-4 border-b dark:border-slate-700"><ImageIcon className="w-4 h-4" /><span className="text-sm mr-auto min-w-0 truncate">{preview.name}</span><a href={preview.rawUrl} target="_blank" rel="noopener noreferrer" className="text-xs underline shrink-0">{t('assets.openOriginal')}</a><button aria-label="Close asset preview" onClick={() => setPreview(null)} className="p-1 rounded hover:bg-black/5 dark:hover:bg-white/10 transition"><X className="w-5 h-5" /></button></div>
