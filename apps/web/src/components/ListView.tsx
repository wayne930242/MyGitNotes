          ...
          <div>
            <div className="font-medium text-fg transition flex items-center gap-1.5">
              ...
              {note.path.endsWith('.mdx') && (
                <span className="text-[10px] font-semibold font-mono px-1.5 py-0.5 rounded bg-warning-soft text-warning border border-warning/40 leading-none">
                  MDX
                </span>
              )}
            </div>
          ...
          {!readOnly && canDelete && (
            <button
              ...
              className={isPendingDelete
                ? 'p-1.5 text-on-danger bg-danger hover:bg-danger/90 rounded-lg transition'
                : 'p-1.5 text-muted hover:text-danger hover:bg-danger-soft rounded-lg transition'}
            >
      ...
      <th className={`py-3 px-4 select-none ${className}`}>
        {onSortChange ? (
          <button
            ...
            className="group inline-flex items-center gap-1 text-xs uppercase font-semibold text-muted hover:text-fg transition"
          >
            <span>{label}</span>
            {isActive ? (
              sortOrder === 'asc' ? (
              ...
              <ArrowUpDown className="w-3.5 h-3.5 opacity-0 group-hover:opacity-60 transition text-muted" />
            )}
          </button>
        ) : (
          <span>{label}</span>
    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead
            className="bg-fg/5 border-b text-xs uppercase font-semibold text-muted"
          ...
          {uncommitted.length > 0 && <tbody className="divide-y note-list-uncommitted" style={{ borderColor: 'var(--color-border)' }}>
            <tr><th colSpan={5} scope="colgroup" className="py-2 px-4 text-left text-xs uppercase font-semibold text-warning">{t('notes.uncommitted')}</th></tr>
          ...
          <tbody className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
            {leading && <tr className="note-list-leading hover:bg-fg/5 transition"><td colSpan={5} className="py-3 px-4">{leading}</td></tr>}
            {/* Notes row rendering */}
