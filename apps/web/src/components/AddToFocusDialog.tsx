...
export const AddToFocusDialog: React.FC<{ focus: NoteFocus; tab: FocusTab; label: string; onClose: () => void }> = ({ focus, tab, label, onClose }) => {
  ...
  const land = () => {
    ...
    if (!from || !to || matchMedia('(prefers-reduced-motion: reduce)').matches) { window.setTimeout(onClose, 1200); return; }
    ...
    ], { duration: 600, easing: 'cubic-bezier(.4, 0, .2, 1)', fill: 'forwards' }).finished.then(onClose, onClose);
  };

  return <WorkspaceDialog title={t('focus.addTo')} onClose={onClose} className="focus-add-dialog">
    <form className="screen-form" onSubmit={async event => {
      ...
      <p className="focus-add-item"><span ref={flight}>{label}</span></p>
      {targets.length > 1 && <label>{t('focus.title')}
        <Select value={target} disabled={placed} onValueChange={value => { setTarget(value); setPane(focus.entryOf(value)?.activePane ?? 0); }}
          options={targets.map(value => ({ value, label: nameOf(value) }))} />
      </label>}
      {layout && <div ref={thumbnail}>
