...
import { ThemeChoice, ThemeMode, resolveMode } from '../lib/themes.js';
        ...
        {PALETTE_FAMILIES.map(family => {
          ...
          return (
            <button
              ...
              onClick={() => onChange({ ...value, familyId: family.id })}
              className={`flex min-w-0 items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs ${selected ? 'bg-primary-soft text-fg font-semibold' : 'text-muted hover:bg-sidebar hover:text-fg'}`}
            >
              <span className="flex shrink-0 overflow-hidden rounded-sm border border-line" aria-hidden="true">
                {[variant.background, variant.text, variant.primary, ...variant.accents.slice(0, 3)].map((color, index) => (
                  <span key={index} className="h-3.5 w-2" style={{ backgroundColor: color }} />
                ))}
              </span>
