  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: isBeingDragged ? 'var(--color-primary)' : 'var(--color-border)',
      }}
    >
...
const columnSortOptionValues = [
  { value: 'updated:desc', label: 'kanban.newest' },
  { value: 'updated:asc', label: 'kanban.oldest' },
  { value: 'title:asc', label: 'kanban.titleAsc' },
  { value: 'title:desc', label: 'kanban.titleDesc' },
  { value: 'created:desc', label: 'kanban.createdNewest' },
] as const;

function KanbanColumn({ col, index, board, query, hiddenNote, sort, onSort }: {
  col: Column; index: number; board: BoardContext; query: Partial<NoteQuery>; hiddenNote?: NoteListItem;
  sort: { field: SortField; order: SortOrder }; onSort: (value: { field: SortField; order: SortOrder }) => void;
}) {
  const { t } = useTranslation();
  ...
  useEffect(() => { report(col.id, result.total); }, [report, col.id, result.total]);
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isColumnDragOver ? '0 0 0 2px var(--color-primary), 0 8px 20px -4px var(--color-primary-light)' : undefined,
      }}
    >
      {/* Column Header */}
      ...
      <div className="overflow-y-auto space-y-2.5 flex-1 pr-0.5 min-h-[80px]">
        ...
        {isColumnDragOver && board.dragged && (
          <div
        ...
        {!result.loading && notes.length === 0 && !isColumnDragOver ? (
          ...
          notes.map(note => <KanbanCard key={note.path} note={note} board={board} index={index} />)
        )}
...
function KanbanUnassignedColumn({ board, query, hiddenNote, sort }: {
  board: BoardContext; query: Partial<NoteQuery>; hiddenNote?: NoteListItem; sort: { field: SortField; order: SortOrder };
}) {
  const { t } = useTranslation();
  ...
  const report = board.onTotal;
  useEffect(() => { report('', result.total); }, [report, result.total]);
      ...
      <div ref={columnsRef} data-kanban-columns="" className="flex gap-5 overflow-x-auto pt-1 px-1 pb-6 flex-1 items-start">
        {columns.map((col, index) => (
          <KanbanColumn
            ...
            sort={columnSorts[col.id] || { field: effectiveBoardSortField, order: sortOrder }}
            onSort={value => setColumnSorts(previous => ({ ...previous, [col.id]: value }))}
          />
        ))}
