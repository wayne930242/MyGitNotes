                ...
                <div className='flex flex-col gap-1'>
                  ...
                  {(nb.metadata ?? []).map((field, index) => (
                    <div key={index} className='flex items-center gap-1.5'>
                      <input type='text' disabled={readOnly} placeholder={t('settings.manifestMetadataKeyPlaceholder')} value={field.key} onChange={e => updateNotebook(nb.id, current => ({ ...current, metadata: (current.metadata ?? []).map((f, i) => i === index ? { ...f, key: e.target.value } : f) }))} className='flex-1 p-1.5 text-xs font-mono bg-surface border border-line rounded-lg focus:outline-none focus:ring-2 focus:ring-primary' />
                      <select disabled={readOnly} value={field.type ?? 'string'} onChange={e => updateNotebook(nb.id, current => ({ ...current, metadata: (current.metadata ?? []).map((f, i) => i === index ? { ...f, type: e.target.value as NotebookMetadataField['type'] } : f) }))} className='p-1.5 text-xs bg-surface border border-line rounded-lg focus:outline-none focus:ring-2 focus:ring-primary'>{METADATA_TYPES.map(type => <option key={type} value={type}>{type}</option>)}</select>
                      <input type='text' disabled={readOnly} placeholder={t('settings.manifestMetadataLabelPlaceholder')} value={field.label ?? ''} onChange={e => updateNotebook(nb.id, current => ({ ...current, metadata: (current.metadata ?? []).map((f, i) => i === index ? { ...f, label: e.target.value } : f) }))} className='flex-1 p-1.5 text-xs bg-surface border border-line rounded-lg focus:outline-none focus:ring-2 focus:ring-primary' />
                      {!readOnly && (
                  ...
                  {Object.entries(nb.pathAliases ?? {}).map(([alias, target], index) => (
                    <div key={index} className='flex items-center gap-1.5'>
                      <input
                        ...
                        className='flex-1 p-1.5 text-xs font-mono bg-surface border border-line rounded-lg focus:outline-none focus:ring-2 focus:ring-primary'
                      />
                      <input
                        type='text'
                        disabled={readOnly}
