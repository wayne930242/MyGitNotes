...
export function R2Panel({ notebookId, listing, directory, mutable, showHidden, busy, run, onNavigate, onRefresh, beforeChange, onNotesChanged, onInsert }: R2PanelProps) {
  ...
  const finish = async (next: string, select = '') => {
    setOperation(undefined);
    await onRefresh();
    if (next === directory) setSelected(select);
    else {
  ...
  return (
    <section className={`file-content ${selectedObject ? 'has-selection' : ''}`}>
      <div className='file-actions'>
        {mutable && (
          <>
            ...
            <label className='ui-button' aria-disabled={busy}>
              <Upload size={15} />
              {t('files.upload')}
              <input
                type='file'
            ...
            {directory !== root && !selectedObject && (
              <>
                ...
                >
                  <FolderInput size={15} />
                  {t('files.r2MoveFolder')}
                </button>
                <button type='button' className='ui-button ui-button-danger' disabled={busy} onClick={() => open('delete')}>
                  <Trash2 size={15} />
                  {t('files.r2DeleteFolder')}
                </button>
              </>
            )}
          </>
      ...
      <div className='file-list' aria-label={t('files.list')}>
        ...
        {files.map(object => (
          <div key={object.key} className={`file-row ${selected === object.key ? 'is-selected' : ''}`}>
            <button
              type='button'
              disabled={busy}
      ...
      {selectedObject && (
        <section className='file-detail' aria-label={t('files.details')}>
          <div className='file-detail-heading'>
            ...
            <code>{`r2:${selectedObject.key}`}</code>
          </div>
          <div className='file-actions'>
            <button
              type='button'
              ...
              disabled={busy}
              onClick={() =>
                void copyToClipboard(r2Reference(selectedObject.key)).then(ok => {
                  if (ok) {
                    setCopied(true);
            ...
            >
              <Copy size={15} />
              {copied ? t('files.r2Copied') : t('files.r2CopyReference')}
            </button>
            {mutable && (
              <>
                <button type='button' className='ui-button' disabled={busy} onClick={() => open('move')}>
                  <FolderInput size={15} />
                  {t('files.move')}
                </button>
                <button type='button' className='ui-button ui-button-danger' disabled={busy} onClick={() => open('delete')}>
      ...
      {operation && mutable && (
        ...
        >
          ...
          {operation !== 'mkdir' && (
            <p>
              <code>{relative(target)}</code>
            </p>
          )}
          {operation !== 'delete' && (
            <label>
              {t('files.name')}
              <input className='ui-control' autoFocus required maxLength={200} pattern='[^/\\\\]+' value={name} disabled={busy} onChange={event => setName(event.target.value)} />
            </label>
          )}
          {operation === 'move' && (
            <>
              <label>
                {t('files.destination')}
                <select className='ui-control' value={destination} disabled={busy} onChange={event => setDestination(event.target.value)}>{destinations.map(folder => <option key={folder} value={folder}>{relative(folder)}</option>)}</select>
              </label>
          ...
          {operation === 'delete' && references && directoryTarget && (
            <>
              <p>{t('files.r2DeleteObjects', { count: references.objects.length })}</p>
              <ul className='file-affected'>
                {references.objects.map(key => (
                  <li key={key}>
                    <code>{key.slice(target.length + 1)}</code>
                  </li>
                ))}
              </ul>
            </>
          )}
          {operation !== 'mkdir' && (references
