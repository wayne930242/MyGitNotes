...
import { escapeHtml, findDirectiveBlocks } from '../lib/directives.js';
import { forwardRef, useEffect, useImperativeHandle, useRef } from 'react';
import { Compartment, EditorState, type Range, StateEffect, StateField, Transaction } from '@codemirror/state';
import { Decoration, type DecorationSet, drawSelection, EditorView, highlightActiveLineGutter, keymap, layer, lineNumbers, RectangleMarker, WidgetType } from '@codemirror/view';
...
import { type I18nContextValue, useTranslation } from '../lib/i18n/index.js';
...
import { DONE_EMOJI, DUE_EMOJI, findToken, isTaskLine, setTaskChecked, setTokenValue, START_EMOJI, TIMESTAMP_EMOJI } from '../lib/task-tokens.js';
...
export const LiveMarkdownEditor = forwardRef<LiveMarkdownHandle, Props>(({ content, notePath, readOnly, onChange, onCaret, ariaLabel = 'Note content', showLineNumbers = true, lineNumberOffset = 0, onCopyLines }, ref) => {
  ...
  useEffect(() => {
    ...
    const gutterMouseDown = (event: MouseEvent) => {
      ...
      gutterDrag.current = { start: line, current: line };
      ...
      if (previous?.line === line && performance.now() - previous.at < 500) {
        lastGutterClick.current = null;
        lastGutterCopy.current = { line, at: performance.now() };
        gutterDrag.current = null;
        clearGutterRange();
        copyLinesCallback.current?.(line);
        return;
      }
      ...
      const up = (upEvent: MouseEvent) => {
        ...
        if (drag.start !== drag.current) {
          lastGutterClick.current = null;
          copyLinesCallback.current?.(drag.start, drag.current);
      ...
      gutterDragCleanup.current = () => {
        ...
        gutterDragCleanup.current = null;
      };
