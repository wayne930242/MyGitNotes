...
export function AssetBrowser({ selectedNotebookId, ...library }: AssetBrowserProps) {
  ...
  return (
    <AssetLibrary
      ...
      initialAssetPath={requestedAsset}
      initialDirectory={new URLSearchParams(location.search).get('directory') ?? undefined}
      {...library}
            ...
            <div>
              ...
              {['', ...folders.filter(Boolean)].map(folder => (
                <button
                  ...
                  disabled={busy}
                  aria-pressed={directory === folder}
                  onClick={() => {
                    onSelectDirectory(folder);
                    sidebar.setOpen(false);
                  }}
                >
                  <Folder aria-hidden='true' className='w-4 h-4' />
                  <span>{folder || t('assets.root')}</span>
                </button>
              ))}
            </div>
          </WorkspaceSidebar>
