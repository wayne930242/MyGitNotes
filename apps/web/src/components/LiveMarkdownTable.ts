...
const icons: Record<string, string> = {
  expand: '<path d="M8 4H4v4m0-4 6 6m6 10h4v-4m0 4-6-6"/>',
  fit: '<path d="M4 4v16M20 4v16M7 12h10m-7-3-3 3 3 3m4-6 3 3-3 3"/>',
  row: '<rect x="3" y="3" width="18" height="14" rx="2"/><path d="M3 8h18M3 12h18M8 21h8"/>',
  column: '<rect x="3" y="3" width="18" height="14" rx="2"/><path d="M9 3v14M15 3v14M8 21h8"/>',
  edit: '<path d="m15 5 4 4M4 20l4-1L20 7a3 3 0 0 0-4-4L4 15v5Z"/>',
};
interface TableUI { row: number; column: number; width: 'fit' | 'expanded' }
...
export class LiveMarkdownTable extends WidgetType {
  constructor(readonly text: string, readonly path: string, readonly from: number, readonly readOnly: boolean, readonly t: Translate, readonly ui?: TableUI) { super(); }
  ...
  toDOM(view: EditorView) {
    ...
    if (!this.ui) { try { width = localStorage.getItem('github-notes:table-width') === 'fit' ? 'fit' : 'expanded'; } catch { /* Use the default width. */ } }
    ...
    const button = (parent: HTMLElement, label: string, symbol: string, action: () => void, className = '') => {
      const control = document.createElement('button'); control.type = 'button'; control.className = className;
      ...
      control.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); action(); });
    ...
    const save = (focusAfter = true) => {
      ...
      view.dispatch({ ...(text === this.text ? {} : { changes: { from: this.from, to: this.from + this.text.length, insert: text } }),
        effects: tableUIChanged.of({ from: this.from, ui: { row, column, width } }), annotations: isolateHistory.of('full'), userEvent: 'input.table' });
      view.requestMeasure(); if (focusAfter) restoreFocus();
    };
    const setWidth = () => {
      width = width === 'fit' ? 'expanded' : 'fit';
      captureInput();
      try { localStorage.setItem('github-notes:table-width', width); } catch { /* Keep this editor's choice. */ }
      if (input) save();
      else view.dispatch({ effects: tableUIChanged.of({ from: this.from, ui: { row, column, width } }) });
      view.requestMeasure(); restoreFocus();
    };
    ...
    const alignment = document.createElement('select'); alignment.title = this.t('table.align'); alignment.setAttribute('aria-label', this.t('table.align'));
    for (const value of ['none', 'left', 'center', 'right'] as const) {
      const option = document.createElement('option'); option.value = value; option.textContent = this.t(`table.${value}`); alignment.append(option);
    }
    alignment.addEventListener('mousedown', event => { if (input) event.preventDefault(); });
    alignment.addEventListener('change', () => { captureInput(); model.alignments[column] = alignment.value as TableAlignment; save(); });
    toolbar.prepend(alignment);
    const deleteRow = button(toolbar, this.t('table.deleteRow'), 'row', () => {
      if (row === 0) return;
      captureInput(); input = null; model.rows.splice(row, 1); row--; save();
    });
    const deleteColumn = button(toolbar, this.t('table.deleteColumn'), 'column', () => {
      if (model.alignments.length === 1) return;
      captureInput(); input = null; model.rows.forEach(values => values.splice(column, 1)); model.alignments.splice(column, 1); column = Math.max(0, column - 1); save();
    });
    ...
    const selectCell = (nextRow: number, nextColumn: number) => {
      ...
      deleteRow.disabled = row === 0; deleteColumn.disabled = model.alignments.length === 1;
      ...
      input.addEventListener('keydown', event => {
        ...
        if (event.key === 'Escape') { event.preventDefault(); cancel(); }
          ...
          if (next >= 0 && next < model.rows.length * model.alignments.length) {
            event.preventDefault(); captureInput(); input = null;
            row = Math.floor(next / model.alignments.length); column = next % model.alignments.length; save();
          }
        }
      });
    ...
    for (const [r, values] of [...table.rows].entries()) for (const [c, cell] of [...values.cells].entries()) {
      ...
      cell.addEventListener('keydown', event => {
        ...
        if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'z') { event.preventDefault(); event.stopPropagation(); (event.shiftKey ? redo : undo)(view); restoreFocus(); }
      });
    ...
    const addRow = button(root, this.t('table.insertRowHere'), '+', () => {
      captureInput(); input = null; model.rows.splice(rowIndex, 0, model.alignments.map(() => '')); row = rowIndex; save();
    }, 'live-table-insert live-table-insert-row');
    const addColumn = button(root, this.t('table.insertColumnHere'), '+', () => {
      captureInput(); input = null; model.rows.forEach(values => values.splice(columnIndex, 0, '')); model.alignments.splice(columnIndex, 0, 'none'); column = columnIndex; save();
    ...
    const positionHandles = (x: number, y: number, showSelected = false) => {
      ...
      addRow.style.left = `${scrollBox.left - box.left}px`; addRow.style.top = `${closestRow.position - box.top}px`;
      ...
      if (closestColumn) {
        ...
        addColumn.style.left = `${closestColumn.position - box.left}px`; addColumn.style.top = `${scrollBox.top - box.top}px`;
    ...
    scroller.addEventListener('scroll', () => {
      ...
      else { addRow.dataset.visible = 'false'; addColumn.dataset.visible = 'false'; }
    });
    return root;
  }
  get estimatedHeight() { return (this.text.split('\n').length - 1) * 40 + 48; }
}
