...
import { BookOpen, Cloud, Code2, CornerLeftUp, Download, Eye, EyeOff, File, FilePen, FilePlus, FileText, FileX, Folder, FolderInput, FolderPlus, Info, Pencil, RefreshCw, Trash2, Upload, X } from 'lucide-react';
import type { FileCommand } from '@mygitnotes/core';
import type { NotebookConfig } from '../lib/types.js';
import { fetchFiles, type FileEntry, type FileListing, type FileRead, type FileResult, mutateFile, rawFileUrl, readFile } from '../lib/files-api.js';
...
import { r2Folders, R2Panel } from './R2Panel.js';
    ...
    <div className='file-manager' data-mode={mode} data-layout={layout} aria-busy={busy || reading || !listing}>
      ...
      <header className='file-manager-toolbar'>
        ...
        <div className='file-toolbar-view' role='group' aria-label={t('files.display')}>
          <button
            type='button'
            className='ui-icon-button file-hidden-toggle'
          ...
          <button
            type='button'
            className='ui-icon-button file-markdown-toggle'
            aria-label={t('files.showMarkdown')}
            title={t('files.showMarkdown')}
                    ...
                    {directory !== listing.root && (
                      <div className='file-row file-navigation-row'>
                        <button
                          ...
                          title={t('files.up')}
                          disabled={busy}
                          onClick={() => void navigate(parentOf(directory))}
                        >
                    ...
                    {current.map(entry => (
                      <div key={entry.path} className={`file-row ${selected === entry.path ? 'is-selected' : ''}`}>
                        <button
                          ...
                          className='file-row-name'
                          title={entry.path}
                          aria-label={`${entry.directory ? t('files.openFolder') : t('files.select')}: ${entry.name}`}
                          onClick={() => void navigate(entry.path, !entry.directory)}
                        >
                        ...
                        {entry.directory && mutable && (
                          <button
                            ...
                            className='ui-icon-button'
                            disabled={busy}
                            aria-label={`${t('files.metadata')}: ${entry.name}`}
                            title={t('files.metadata')}
                            onClick={() => void navigate(entry.path, true)}
                          >
                            <Pencil size={15} />
                          </button>
                        )}
                        ...
                        {mutable && (
                          <>
                            ...
                            <button
                              ...
                              disabled={busy}
                              onClick={() => void openOperation(selectedEntry.directory ? 'remove-directory' : 'delete')}
                            >
                              <Trash2 size={15} />
                              {t('common.delete')}
                            </button>
                    ...
                    >
                      ...
                      {(operation === 'move' || operation === 'remove-directory') && (
                        <label>
                          ...
                          >
                            {destinationDirs.map(dir => <option key={dir.path} value={dir.path}>{relative(dir.path)}</option>)}
                          </select>
                        </label>
                      )}
                      {operation === 'move' && (
      ...
      {confirmLeave && (
        <WorkspaceDialog title={t('files.unsaved')} onClose={() => finishLeave(false)}>
