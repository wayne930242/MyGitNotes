...
import { Plus, GripVertical, ArrowLeft, Pencil } from 'lucide-react';
...
import { screenItemTitle, type ScreenContentProps } from './ScreenCard.js';
...
export function ScreenPage({ notebooks, folders, selectedNotebookId, screen, onOpenNote, onStudySaved, focusedLaneId, onCreateNote, focusSection, onAddLaneToFocus }: {
  ...
  focusSection?: ReactNode; onAddLaneToFocus?: (row: ScreenRow) => void;
}) {
  ...
  return <div className={`workspace-route screen-main ${focusedLaneId ? 'screen-focused' : 'has-sidebar-drawer'}`}>
    ...
    <main className="screen-content">
      ...
      {assetError && <div role="alert" className="screen-error">{t('screen.assetsError')}
        <Button type="button"  aria-label={t('screen.retryAssets')} disabled={assetsLoading} onClick={() => retryAssets()}>{t('screen.retryAssets')}</Button>
      </div>}
      ...
      {screen.loading ? <p role="status">{t('screen.loading')}</p> : <>
        ...
        {focusedLaneId ? reviewRow && <section id={`screen-lane-${reviewRow.id}`} className="screen-study-session" aria-label={reviewRow.name}>
          ...
          {rows.map(row => <ScreenLane reorder={reorder} key={row.id} row={row} {...content} facets={facets.facets} disabled={disabled} study={study}
            graph={row.view === 'graph' ? <GraphPage notebooks={notebooks} lane={row} screen={screen} /> : undefined}
            onCreateNote={onCreateNote}
            onAddToFocus={onAddLaneToFocus ? () => onAddLaneToFocus(row) : undefined}
