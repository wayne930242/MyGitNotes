...
export function useScreenItemOpen(options: {
  notebooks: NotebookConfig[]; assets: ScreenAsset[]; onOpenNote: (note: NoteListItem) => void; onMissing: () => void;
}): { open: ScreenContentProps['onOpen']; preview: ReactNode } {
  ...
  const open: ScreenContentProps['onOpen'] = (item, note) => {
    ...
    if (!nb) { onMissing(); return; }
    if (item.kind === 'note') {
      if (note) onOpenNote(note); else onMissing();
    ...
    else { const asset = assets.find(asset => asset.notebookId === item.notebookId && asset.path === item.path); if (asset) setPreview(asset); else onMissing(); }
  };
  const previewNode: ReactNode = preview ? <WorkspaceDialog title={preview.name} onClose={() => setPreview(undefined)} className="asset-preview-dialog">
    ...
    <div className="workspace-dialog-actions"><Button onClick={() => navigate(`/assets?notebook=${encodeURIComponent(preview.notebookId)}&asset=${encodeURIComponent(preview.path)}`)}>{t('links.locateAsset')}</Button></div>
