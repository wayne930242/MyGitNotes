...
export function GraphPage({ notebooks, filters, screen, lane, folders = [] }: GraphPageProps) {
  ...
  return (
    <div
      ref={container}
      style={lane ? { height: laneViewport.height } : undefined}
      data-selected-count={visibleSelected.length}
      tabIndex={0}
      aria-label={t('graph.canvasHelp')}
      className={`graph-page-container graph-editing-surface ${lane ? 'graph-in-lane' : ''}`}
    ...
    >
      ...
      {saveOpen && (
        <WorkspaceDialog title={t('graph.saveLane')} onClose={() => setSaveOpen(false)}>
