...
export function AgentAccessSettings({ local = false }: { local?: boolean }) {
  ...
  return <section className="border border-line rounded-xl p-4 flex flex-col gap-3">
    <div className="flex items-center justify-between gap-2">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1 text-xs rounded-lg border border-line bg-surface hover:bg-sidebar text-fg transition active:scale-95 shadow-xs shrink-0 cursor-pointer"
    ...
    {local ? <p role="status" className="text-sm text-muted">{t('auth.localAccessNotice')}</p> : session.provider && !session.authenticated && <p className="text-sm"><a className="underline" href={session.provider === 'gitlab' ? '/api/auth/gitlab' : '/api/auth/github'}>{t(session.provider === 'gitlab' ? 'auth.signInWithGitlab' : 'auth.signInWithGithub')}</a>{t('auth.signInToManage')}</p>}
    <>
      <div className="flex flex-wrap gap-3 items-end">
        <label className="text-xs text-muted flex flex-col gap-1">{t('auth.clientName')}<input disabled={!canManage || busy} aria-label={t('auth.clientName')} maxLength={80} value={name} onChange={e => setName(e.target.value)} placeholder={t('auth.clientNamePlaceholder')} className="px-3 py-2 rounded-lg border border-line bg-transparent" /></label>
        <label className="text-xs text-muted flex flex-col gap-1">{t('auth.access')}<Select disabled={!canManage || busy} aria-label={t('auth.access')} value={write ? 'write' : 'read'} onValueChange={value => setWrite(value === 'write')} options={[{ value: 'read', label: t('auth.readOnly') }, { value: 'write', label: t('auth.readAndWrite') }]} /></label>
        <Button variant="primary" disabled={!canManage || busy} onClick={create}><Plus className="w-3.5 h-3.5" />{t('auth.createGrant')}</Button>
      </div>
      {canManage && token && <div className="p-3 rounded-lg bg-fg/5 flex flex-col gap-2">
        ...
        <div className="flex gap-2 items-center flex-wrap sm:flex-nowrap">
          <input
            ...
            className="flex-1 min-w-0 p-2 rounded border border-line bg-transparent font-mono text-xs cursor-pointer select-all focus:outline-none focus:ring-1 focus:ring-primary"
          />
...
>
          ...
          <button
            type="button"
            className="px-2.5 py-1.5 text-xs rounded-lg border border-line bg-surface hover:bg-sidebar text-fg flex items-center gap-1.5 transition active:scale-95 shadow-xs shrink-0 cursor-pointer"
            onClick={() => setIsTutorialOpen(true)}
          >
            <HelpCircle className="w-3.5 h-3.5 text-primary" />
            <span>{t('auth.connectorGuide')}</span>
      ...
      {canManage && <div className="divide-y divide-line">{grants.map(grant => <div key={grant.id} className="py-3 flex items-center justify-between gap-3">
        <div className="min-w-0"><p className="text-sm font-medium truncate">{grant.name}</p><p className="text-xs text-muted">{grant.write ? t('auth.readAndWrite') : t('auth.readOnly')} · {t('auth.untilRevoked')} · {new Date(grant.createdAt).toLocaleDateString(language)}</p><p className="text-xs font-mono text-muted truncate">{grant.source.replace(/^(github|gitlab):/, '')}</p></div>
        {confirmRevoke === grant.id ? <div className="flex gap-3 text-xs shrink-0"><button disabled={busy} className="text-danger font-semibold hover:underline disabled:opacity-40 disabled:cursor-not-allowed transition" onClick={() => revoke(grant.id)}>{t('auth.confirmRevoke')}</button><button onClick={() => setConfirmRevoke(null)} className="hover:underline transition">{t('common.cancel')}</button></div> : <button disabled={busy} className="text-xs text-danger hover:text-danger flex items-center gap-1 shrink-0 disabled:opacity-40 disabled:cursor-not-allowed transition" onClick={() => setConfirmRevoke(grant.id)}><Trash2 className="w-3.5 h-3.5" />{t('auth.revoke')}</button>}
      </div>)}{!grants.length && <p className="text-xs text-muted py-2">{t('auth.noGrants')}</p>}</div>}
    </>
