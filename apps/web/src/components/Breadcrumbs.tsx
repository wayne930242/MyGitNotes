...
export const Breadcrumbs: React.FC<BreadcrumbsProps> = ({
  ...
  return (
    <nav
      ...
      className="workspace-breadcrumbs flex items-center justify-between gap-3 px-3 py-2 md:px-4 md:py-2.5 rounded-xl border bg-fg/[0.02] mb-4 text-xs select-none transition-colors flex-wrap sm:flex-nowrap"
    ...
    >
      <div className="flex items-center gap-1.5 flex-wrap min-w-0">
        {currentFolder !== null && (
          <button
            ...
            className="inline-flex items-center gap-1 px-2 py-1 -ml-1 rounded-lg text-muted hover:text-fg hover:bg-fg/5 transition active:scale-95 shrink-0"
          >
          ...
          {segments.filter(seg => seg.path !== null).map((seg, idx, shown) => {
            ...
            return (
              <React.Fragment key={seg.path}>
                {idx > 0 && (
                  <ChevronRight className="w-3.5 h-3.5 text-muted shrink-0 mx-0.5" />
                )}
      ...
      <div className="flex items-center gap-3 shrink-0 justify-between sm:justify-end w-full sm:w-auto pt-1 sm:pt-0 border-t sm:border-t-0 border-line">
        ...
        <div className="flex items-center gap-1.5 text-muted text-[11px] font-mono">
          {subfolderCount > 0 && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-fg/5 text-muted">
