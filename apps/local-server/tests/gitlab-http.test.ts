...
async function login() {
  const start = await fetch(`${base}/api/auth/gitlab`, { redirect: 'manual' });
  expect(start.status).toBe(302);
  const target = new URL(start.headers.get('location')!);
  expect(target.origin + target.pathname).toBe(`${site}/oauth/authorize`);
  expect(target.searchParams.get('scope')).toBe('api');
  ...
  const session = callback.headers.getSetCookie().find(s => s.startsWith('gh_notes_session='))!;
  expect(session).toContain('HttpOnly');
  expect(session).not.toContain('fixture-token');
  cookie = session.split(';')[0];
  return { state: target.searchParams.get('state'), oauthCookie };
}
beforeEach(async () => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'mygitnotes-gitlab-'));
  refreshes = 0;
  cookie = '';
  fixture = gitlabFixture(site);
  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    ...
    if (url === `${site}/oauth/token`) {
      ...
      expect(body.client_id).toBe('fixture-client');
      expect(body.redirect_uri).toBe(`${base}/api/auth/gitlab/callback`);
      expect(init?.redirect).toBe('error');
      if (body.grant_type === 'refresh_token') {
        refreshes++;
        expect(body.refresh_token).toBe('fixture-refresh');
        return json({ access_token: 'refreshed-token', refresh_token: 'rotated-refresh', expires_in: 7200 });
      }
...
describe('GitLab HTTP and MCP integration', () => {
  ...
  it('refreshes one shared credential for concurrent browser/MCP calls and preserves grants after logout until revocation', async () => {
    ...
    const listing = await fetch(grant.url, mcp('tools/list')).then(r => r.json());
    expect(listing.result.tools.some((t: any) => t.name === 'write')).toBe(false);
    ...
    await fetch(`${base}/api/auth/logout`, post({}));
    expect((await fetch(grant.url, mcp('tools/list'))).status).toBe(200);
    await login();
    const grants = await fetch(`${base}/api/auth/agent-tokens`, { headers: { Cookie: cookie } }).then(r => r.json());
    expect(grants.grants[0].id).toBe(grant.id);
    expect(JSON.stringify(grants)).not.toContain(grant.token);
    expect(JSON.stringify(grants)).not.toContain('fixture-token');
  ...
  it('executes a write grant through MCP and rejects cross-site credential reuse', async () => {
    await login();
    const grant = await fetch(`${base}/api/auth/agent-token`, post({ write: true })).then(r => r.json());
...
it('serializes hosted refresh across independent local lock scopes using Redis and reports refresh failure', async () => {
  ...
  vi.spyOn(SessionStore.prototype, 'command').mockImplementation(async ([command, key, ...args]) => {
    ...
    if (command === 'EVAL') {
      const lock = args[1], nonce = args[2];
      if (values.get(lock) === nonce) return Number(values.delete(lock));
      return 0;
    }
