...
describe('GitHub login and shared agent authorization',()=>{
  ...
  it('logs a secret-free reason for each rejected grant or credential',async()=>{
    ...
    expect(await reasons(grant.url)).toEqual([401,expect.stringMatching(/^\[auth\] record [a-f0-9]{8} sealed with another SESSION_SECRET$/),'[auth] credential rejected: missing']);
