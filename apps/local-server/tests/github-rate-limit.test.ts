...
it('returns a Retry-After header and shares the upstream cooldown across HTTP requests', async () => {
  ...
  vi.stubEnv('VERCEL', ''); vi.stubEnv('APP_URL', '');
  const original = globalThis.fetch; let upstream = 0;
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    if (String(input).startsWith('https://api.github.com/')) { upstream++; return new Response('{}', {status:429, headers:{'Retry-After':'120'}}); }
  ...
  try {
    for (const endpoint of ['/api/workspace', '/api/notes']) {
      ...
      expect(response.status).toBe(429); expect(Number(response.headers.get('Retry-After'))).toBeGreaterThanOrEqual(119);
