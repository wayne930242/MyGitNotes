...
beforeEach(() => {
  ...
  git('add', '.');
  git('commit', '-m', 'content-only main');
  git('push', 'origin', 'main');
  git('checkout', '-qf', 'core');
  git('clean', '-fdq');
  write('product.txt', 'new core\n');
  write('examples/demo-workspace/notes/example/welcome.md', '# Updated tutorial\n');
  git('add', '.');
  git('commit', '-m', 'core update');
  git('push', 'origin', 'core');
});
...
describe('release transaction', () => {
  ...
  it('refuses a main that still tracks Core product paths', () => {
    git('checkout', '-q', 'main');
    write('product.txt', 'product\n');
    git('add', 'product.txt');
    git('commit', '-qm', 'fork-model main');
  ...
  it('keeps main content-only, syncs examples and deploys the Core revision', () => {
    ...
    git('add', '.');
    git('commit', '-qm', 'retired template');
    git('push', '-q', 'origin', 'core');
    const first = run();
    ...
    git('rm', '-q', 'examples/demo-workspace/notes/example/retired.md', 'examples/demo-workspace/.github-notes-screen.yaml');
