...
const names = ['MYGITNOTES_SOURCE', 'MYGITNOTES_REPOSITORY', 'MYGITNOTES_BRANCH', 'MYGITNOTES_GITLAB_URL', 'GITLAB_CLIENT_ID', 'GITLAB_CLIENT_SECRET', 'GITHUB_CLIENT_ID', 'GITHUB_CLIENT_SECRET', 'GITHUB_APP_TYPE', 'APP_URL', 'SESSION_SECRET', 'MYGITNOTES_SESSION_NAMESPACE', 'UPSTASH_REDIS_REST_URL', 'UPSTASH_REDIS_REST_TOKEN', 'GEMINI_API_KEY', 'MYGITNOTES_R2_ACCOUNT_ID', 'MYGITNOTES_R2_ACCESS_KEY_ID', 'MYGITNOTES_R2_SECRET_ACCESS_KEY', 'MYGITNOTES_R2_BUCKET'];
...
for (const name of names) {
  const value = process.env[name] ?? (name.startsWith('MYGITNOTES_') ? process.env[name.replace('MYGITNOTES_', 'GITHUB_NOTES_')] : undefined) ?? (name === 'MYGITNOTES_GITLAB_URL' ? process.env.GITLAB_URL : undefined);
  if (!value) continue;
