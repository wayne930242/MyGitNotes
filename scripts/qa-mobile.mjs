...
const fits = async (selector, minWidth = 0) => {
  ...
  const viewport = await page.evaluate(() => ({ width: innerWidth, height: visualViewport?.height || innerHeight }));
...
const click = async text => {
  if (['Source', 'Live Preview'].includes(text)) {
    ...
    if (mobile && await mobile.evaluate(e => e.getBoundingClientRect().width > 0)) {
      await chooseSelect(page, 'button[role="combobox"][aria-label="Editor mode"]', text === 'Source' ? 'raw' : 'live');
      return;
    }
  }
...
try {
  ...
  for (const width of [320, 390, 430, 820, 1440]) {
    ...
    if (width < 1101) {
      ...
      if (width < 768) {
        const nav = await bounds('nav');
        assert(nav.bottom === 844 && nav.height >= 64, 'Mobile navigation is not at the bottom');
      ...
      if (width < 768) {
        const row = await bounds('.note-list tbody tr');
        assert(row.height <= 72, 'Mobile List is not compact');
    ...
    if (width >= 768 && width <= 1100) {
      ...
      await page.waitForSelector('[data-live-markdown] .cm-lineNumbers');
      await fits('button[aria-label="Close note"]');
      await fits('button[aria-label="Document tools"]');
      await click('Line Numbers');
    ...
    if (width < 768) {
      ...
      const close = await bounds('button[aria-label="Close note"]');
      assert(close.width >= 44 && close.height >= 44, 'Small Close target');
      ...
      await page.waitForFunction(() => !document.querySelector('[data-live-markdown] .cm-lineNumbers'));
      if (width === 320) {
        await tap('.note-panel-tabs [role="tab"][aria-label="Find in note"]');
        await fits('.note-document-panel[data-panel="find"]');
        await fits('.note-find-field');
        await page.keyboard.press('Escape');
        await tap('.note-panel-tabs [role="tab"][aria-label="Outline"]');
        await fits('.note-document-panel[data-panel="outline"]');
    ...
    await fits('[data-markdown-editor]', Math.min(width - 40, 500));
    if (width < 768) {
      await fits('button[role="combobox"][aria-label="Agent document"]');
      assert((await bounds('.agent-toolbar')).height <= 64, 'Agent toolbar wraps on mobile');
    }
  ...
  await tap('button[role="combobox"][aria-label="Editor mode"]');
  await page.waitForSelector('[role="listbox"]');
  await fits('[role="listbox"]');
  await page.keyboard.press('Escape');
  await page.waitForFunction(() => !document.querySelector('[role="listbox"]'));
  assert(await page.$('[aria-label="Note editor"]'), 'Select Escape closed the note');
  await click('Restore Draft');
  await waitDisk('notes/example/root.md', 'Recovered mobile draft');
  await page.waitForFunction(() => !document.querySelector('.editor-notice-actions'));
  await tap('.note-panel-tabs [role="tab"][aria-label="File Git status"]');
  await page.waitForSelector('.note-document-panel[data-panel="git"]');
  await tap('button[aria-label="Restore note"]:not(:disabled)');
  await fits('button[aria-label="Confirm restore note"]');
  assert((await bounds('.note-document-panel')).width <= 320, 'Note restore confirmation overflows panel');
  await tap('button[aria-label="Confirm restore note"]');
  await page.waitForFunction(() => !document.querySelector('.cm-content')?.innerText.includes('Recovered mobile draft'));
  await page.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  ...
  await swipe([120, 250], [280, 250]);
  await page.waitForSelector('[data-responsive-sidebar].is-open');
  await swipe([200, 250], [30, 251]);
  await page.waitForFunction(() => !document.querySelector('[data-responsive-sidebar].is-open'));
  await swipe([240, 250], [380, 250]);
  assert(!await page.$('[data-responsive-sidebar].is-open'), 'Swipe starting in the right half opened sidebar');
  await swipe([12, 250], [14, 350]);
  assert(!await page.$('[data-responsive-sidebar].is-open'), 'Vertical swipe opened sidebar');
  await swipe([12, 250], [42, 250]);
  assert(!await page.$('[data-responsive-sidebar].is-open'), 'Short swipe opened sidebar');
  await swipe([12, 250], [190, 250], true);
  assert(!await page.$('[data-responsive-sidebar].is-open'), 'Cancelled swipe opened sidebar');
  ...
  for (const route of ['/agent?notebook=example', '/assets?notebook=example', '/settings', '/screen?notebook=example']) {
    ...
    await page.touchscreen.tap(b.right - 20, 250);
    await page.waitForFunction(() => !document.querySelector('[data-responsive-sidebar].is-open'));
  }
  ...
  await tap('button[aria-label="Notebooks and filters"]');
  await page.waitForFunction(() => document.querySelector('#notebook-panel').getBoundingClientRect().left >= 0);
  await tap('#notebook-panel .folder-tree-item .nav-tree-entry[title^="projects"]');
  await page.touchscreen.tap(370, 250);
  await page.waitForFunction(() => !document.querySelector('[data-responsive-sidebar].is-open'));
  ...
  await page.waitForSelector('.right-panel-rail [role="tab"][aria-label="Changes"] .right-panel-badge');
  ...
  for (const view of ['list', 'card']) {
    await chooseSelect(page, 'button[role="combobox"][aria-label="Note view"]', view);
    await page.waitForFunction(
      expected => {
        const current = new URL(location.href).searchParams.get('view');
        return expected === 'list' ? !current || current === 'list' : current === expected;
      },
      {},
      view,
    );
    await page.waitForSelector('button[role="combobox"][aria-label="Status for Root Note"]');
    await tap('button[role="combobox"][aria-label="Status for Root Note"]');
    await page.waitForSelector('[role="listbox"]');
    await fits('[role="listbox"]');
    ...
    const a = lum(option.color), b = lum(option.background);
    assert((Math.max(a, b) + 0.05) / (Math.min(a, b) + 0.05) >= 4.5, 'Low contrast dark status options');
    ...
    const c = lum(input.caret), d = lum(input.background);
    assert((Math.max(c, d) + 0.05) / (Math.min(c, d) + 0.05) >= 4.5, 'Dark input caret lacks contrast');
  }
  console.log('PASS dark List/Card select option contrast');
  await chooseSelect(page, 'button[role="combobox"][aria-label="Note view"]', 'kanban');
  await page.waitForSelector('[data-notepath]');
  await tap('button[aria-label="Settings"]');
  await page.waitForSelector('input[aria-label="MCP client name"]');
  await page.$eval('input[aria-label="MCP client name"]', e => e.scrollIntoView());
  await fits('input[aria-label="MCP client name"]');
  await tap('button[aria-label="Settings"]');
  ...
  await click('Notes');
  await tap('button[aria-label="New Note"]');
  ...
  await tap('.note-panel-tabs [role="tab"][aria-label="Frontmatter"]');
  await page.waitForSelector('.note-document-panel[data-open="false"]');
  ...
  await fits('[aria-label="Close note"]');
  await fits('[data-markdown-editor]', 350);
  ...
  await tap('.note-panel-tabs [role="tab"][aria-label="Insert image"]');
  ...
  await tap('.note-document-panel[data-panel="assets"] button[aria-label="Select file: pixel.png"]');
  await page.waitForSelector('.note-document-panel[data-panel="assets"] .file-detail img');
  await click('Insert image');
  await page.waitForSelector('.note-document-panel[data-open="false"]');
  await waitDisk(noteFile, '/raw-assets/by-hash/');
  await tap('[aria-label="Close note"]');
  await page.waitForFunction(() => !document.querySelector('[aria-label="Note editor"]'));
  await tap('button[aria-label="Agent System"]');
  await page.waitForSelector('.cm-content[aria-label="Agent document content"]');
  await page.setViewport({ width: 320, height: 420, isMobile: true, hasTouch: true });
  await tap('[data-sidebar-toggle]');
  await page.waitForSelector('[data-responsive-sidebar].is-open');
  await page.waitForFunction(() => document.querySelector('[data-responsive-sidebar]').getBoundingClientRect().left >= 0);
  await fits('[data-responsive-sidebar]', 200);
  await page.keyboard.press('Escape');
  await page.waitForFunction(() => !document.querySelector('[data-responsive-sidebar].is-open'));
  await fits('[data-markdown-editor]', 250);
  const reducedAgentEditor = await bounds('[data-markdown-editor]');
  assert(reducedAgentEditor.height >= 100, `Agent toolbar consumes reduced viewport: ${JSON.stringify(reducedAgentEditor)}`);
  await page.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  ...
  await page.waitForSelector('button[aria-label="Restore"]:not(:disabled)');
  ...
  await click('Files');
  await page.waitForSelector('.file-manager-toolbar');
  await fits('.file-manager-toolbar');
  ...
  page.on('request', request => {
    ...
    if (url.pathname === '/api/notes') {
      if (request.method() === 'POST') {
        ...
        Object.assign(remoteNote, saved, { revision: 'two' });
  ...
  await tap('[aria-label="Close note"]');
  await tap('.right-panel-rail [role="tab"][aria-label="Changes"]');
  await tap('button[aria-label="Commit notes/example/remote.md"]');
  await page.waitForSelector('dialog[aria-label="Changes"]');
  ...
  await click(commitLabel);
  await page.waitForFunction(() => !document.querySelector('dialog[aria-label="Changes"]'));
  assert(saved?.content.includes('Mobile remote save') && saved.revision === 'one', 'Remote save lost content or revision');
  await page.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  await page.waitForFunction(() => document.querySelector('nav').getBoundingClientRect().bottom === 844);
  await tap('button[aria-label="Settings"]');
  await page.waitForSelector('input[aria-label="MCP client name"]');
  assert(!await page.$eval('input[aria-label="MCP client name"]', e => e.disabled), 'Authenticated access disabled');
  ...
  console.log('PASS mobile GitHub revision-aware save, account controls and Settings-only access; no runtime errors');
} catch (error) {
  console.log(await page.evaluate(() => ({ url: location.href, text: document.body.innerText.slice(-2000) })));
  throw error;
} finally {
  await browser.close();
