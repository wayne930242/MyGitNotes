...
try {
 ...
 await page.click('button[title="Card View"]');await page.waitForFunction(()=>!document.querySelector('tbody'));
 ...
 await chooseSelect(page,selector('Case Note'),'published');await waitDisk('notes/research/other.md','status: published');
 ...
 await chooseSelect(page,selector('Research Note'),'published');await waitDisk('notes/research/review.md','status: published');
 ...
 console.log(await page.evaluate(()=>({url:location.href,text:document.body.innerText.slice(-1500)})));throw error;
