...
try {
  ...
  await page.click('.file-hidden-toggle input'); await choose('.hidden.txt');
  ...
  await page.click('.file-hidden-toggle input'); assert(!await page.$('[aria-label="Select file: .hidden.txt"]'));
  await folder('one'); await choose('config.json'); await source('{"after":true}\n');
  await click('Up'); await page.waitForSelector('dialog[aria-label="Unsaved changes"]');
  ...
  await source('New file content\n'); await click('Up'); await click('Discard changes', 'dialog');
  ...
  await click('Up'); await page.click('[aria-label="Folder information: created"]');
  ...
  await visit('/files?notebook=a'); await page.waitForFunction(() => document.body.textContent.includes('顯示隱藏檔案'));
