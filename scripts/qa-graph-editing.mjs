...
const click = async (text, within='') => { const elements=await page.$$(`${within} button`);for(const element of elements)if(await element.evaluate((el,text)=>(el.textContent.trim()===text||el.getAttribute('aria-label')===text)&&!el.disabled,text)){await element.click();return;}throw Error('Missing button '+text); };
...
try {
 ...
 await page.click('.graph-note-selector label:nth-child(1) input');await page.click('.graph-note-selector label:nth-child(2) input');
 await click('Select notes (2)');await click('Expand notes');
 await page.waitForSelector(cardA+' .cm-content');await page.waitForSelector(cardB+' .cm-content');
 ...
 await page.click('button[aria-label="Reset view"]');await pause(500);
 const expandedBounds = await page.$$eval('[data-graph-note]',cards=>cards.map(card=>{const r=card.getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom};}));
 ...
 await page.click(cardA+' textarea');await page.keyboard.down('Control');await page.keyboard.press('KeyA');await page.keyboard.up('Control');await page.keyboard.type('Alpha edited. ');
 ...
 await page.focus(cardA+' .cm-content');await page.keyboard.down('Control');await page.keyboard.press('End');await page.keyboard.up('Control');await page.keyboard.type('\n[delta](Delta');
 await page.waitForFunction(()=>document.querySelector('.cm-tooltip-autocomplete')?.textContent.includes('Delta'));await pause(200);await page.keyboard.press('Enter');
 await waitFile('notes/a/a.md','[delta](../b/d.md)');
 console.log('PASS live-mode note completion');
 await click('Save as lane');await page.waitForSelector('dialog[open]');await page.type('input[aria-label="Swimlane name"]','Graph selection');await click('Save as lane','dialog[open]');
 let saved;for(let i=0;i<60;i++){saved=parse(fs.readFileSync(path.join(root,'.github-notes-screen.yaml'),'utf8')).rows.find(row=>row.name==='Graph selection');if(saved)break;await pause(100);}
 assert(saved);assert.equal(saved.items.length,2);assert.equal(saved.graph.nodes.filter(n=>n.expanded).length,2);
 await go('/graph?notebook=all&lanes='+saved.id);await page.waitForSelector(cardA);assert.equal(await page.$$eval('[data-graph-note]',cards=>cards.length),2);
 await click('Select notes (0)');await page.click('.graph-note-selector label:nth-child(1) input');await click('Select notes (1)');await click('Collapse notes');
 ...
 await page.setViewport({width:1500,height:1050});await go('/graph?notebook=all&lanes='+saved.id);
 await page.waitForSelector(cardB);await page.click('button[aria-label="Reset view"]');await pause(400);
 const beforeMove=await (await page.$(cardB)).boundingBox(), header=await (await page.$(cardB+' .graph-note-title')).boundingBox();
 await page.mouse.move(header.x+20,header.y+header.height/2);await page.mouse.down();await page.mouse.move(header.x+100,header.y+header.height/2+40,{steps:8});await page.mouse.up();await pause(200);
 const afterMove=await (await page.$(cardB)).boundingBox();assert(Math.abs(afterMove.x-beforeMove.x-80)<5);assert(Math.abs(afterMove.y-beforeMove.y-40)<5);
 const handle=await (await page.$(cardB+' .graph-card-resize')).boundingBox();
 await page.mouse.move(handle.x+handle.width/2,handle.y+handle.height/2);await page.mouse.down();await page.mouse.move(handle.x+handle.width/2+60,handle.y+handle.height/2+40,{steps:8});await page.mouse.up();await pause(200);
 const afterResize=await (await page.$(cardB)).boundingBox();assert(afterResize.width>afterMove.width+50);assert(afterResize.height>afterMove.height+30);
 console.log('PASS note dragging and resizing');
 await go('/graph?notebook=all');await pause(1600);await click('Box select');
 const surface=await (await page.$('.graph-page-container')).boundingBox();
 await page.mouse.move(surface.x+10,surface.y+150);await page.mouse.down();await page.mouse.move(surface.x+surface.width-10,surface.y+surface.height-10,{steps:10});await page.mouse.up();
 await click('Select notes (4)');await click('Select notes (4)');await click('Expand notes');await pause(500);
 const boxes=await page.$$eval('[data-graph-note]',cards=>cards.map(card=>{const r=card.getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom};}));
 assert.equal(boxes.length,4);
 for(let i=0;i<boxes.length;i++)for(let j=i+1;j<boxes.length;j++){const a=boxes[i],b=boxes[j];assert(a.right<=b.left||b.right<=a.left||a.bottom<=b.top||b.bottom<=a.top);}
 ...
 await click('Choose or edit a swimlane');await page.select('.graph-lane-panel select',saved.id);
 await page.waitForFunction(()=>document.querySelector('[data-graph-nodes]')?.dataset.graphNodes==='2');
 await click('Minimize swimlane panel');assert.equal(await page.$eval('.graph-lane-label',el=>el.textContent),'Graph selection');assert.equal(await page.$$('.graph-lane-panel').then(els=>els.length),0);
 assert.equal(await page.$eval('button[aria-label="Choose or edit a swimlane"]',el=>el.getAttribute('aria-pressed')),'true');
 assert(await page.$eval('button[aria-label="Choose or edit a swimlane"]',el=>{const probe=document.createElement('span');probe.style.color='var(--color-primary)';el.append(probe);const same=getComputedStyle(probe).color===getComputedStyle(el).color;probe.remove();return same;}));
 await page.click('.graph-lane-label');
 const savedLayoutResponse=page.waitForResponse(response=>response.url().endsWith('/api/screen-page')&&response.request().method()==='PUT');await click('Save swimlane','.graph-lane-panel');
 const savedLayoutRecord=await (await savedLayoutResponse).json();const savedLayoutRow=savedLayoutRecord.page.rows.find(row=>row.id===saved.id);assert(savedLayoutRow.graph.nodes.some(node=>node.path==='notes/a/b.md'&&node.expanded&&node.width>360));
 ...
 await page.waitForSelector(embedded+' '+cardB);await page.$eval(embedded,el=>el.scrollIntoView({block:'start'}));await pause(500);
 await page.click('.workspace-sidebar-toggle');await pause(250);
 assert(await page.$eval('.workspace-responsive-sidebar.is-open',el=>{const r=el.getBoundingClientRect();return Boolean(document.elementFromPoint(r.left+50,r.top+180)?.closest('.workspace-responsive-sidebar'));}));
 await page.click('.workspace-sidebar-toggle');await pause(250);
 ...
 await page.mouse.move(ax,ay);await page.waitForSelector(embedded+' .graph-hover-expand');await page.click(embedded+' .graph-hover-expand');await page.waitForSelector(embedded+' '+cardA);
 await pause(900);
 const expandedHeight=await page.$eval(embedded+' .graph-page-container',el=>el.clientHeight);
 await page.click(embedded+' '+cardA+' button[aria-label="Collapse notes"]');await pause(1000);
 ...
 await page.mouse.move(currentB.x+currentB.width/2+(latestA.x-latestB.x)*currentK,currentB.y+currentB.height/2+(latestA.y-latestB.y)*currentK);await page.waitForSelector(embedded+' .graph-hover-expand');
 console.log('PASS embedded canvas grows on expansion and shrinks on collapse');
 const hoverButton=await (await page.$(embedded+' .graph-hover-expand')).boundingBox();await page.mouse.click(hoverButton.x-14,hoverButton.y+14,{delay:80});await page.waitForSelector(embedded+' [data-selected-count="1"]');await page.keyboard.press('Enter');await page.waitForSelector(embedded+' '+cardA);
 await page.click(embedded+' .graph-fullscreen-tool button');await page.waitForFunction(id=>location.pathname==='/graph'&&new URLSearchParams(location.search).get('lanes')===id,{},saved.id);
 console.log('PASS embedded hover and Enter expansion, hidden toolbars, full-page swimlane navigation');
 await page.waitForFunction(()=>{const cards=[...document.querySelectorAll('[data-graph-note]')];return cards.length===2&&cards.every(card=>card.getBoundingClientRect().width<650);});await pause(300);
 ...
 await page.click(cardA+' button[aria-label="Collapse notes"]');assert.equal(await page.$$eval('.is-closing',els=>els.length),0);
...
} catch(error) { await page.screenshot({path:`${product}/artifacts/qa/graph-editing-failure.png`,fullPage:true});console.error('PAGE ERRORS',errors);throw error; }
