...
const click = async text => {
  ...
  if (file) {
    await page.waitForFunction(file => [...document.querySelectorAll('.agent-file')].some(button => button.title.split('\n')[0] === file && !button.disabled), {}, file);
    await page.evaluate(file => [...document.querySelectorAll('.agent-file')].find(button => button.title.split('\n')[0] === file).click(), file);
    return;
  }
...
const append = async (selector, text) => {
  await page.focus(selector);
  await page.keyboard.down(mod);
  await page.keyboard.press('End', { commands: ['MoveToEndOfDocument'] });
...
page.on('request', request => {
  ...
  if (url.pathname === '/api/agent-resources/save') {
    ...
    if (failSave) {
      void request.respond({ status: 500, contentType: 'application/json', body: JSON.stringify({ error: 'Fixture save failure' }) });
      return;
    }
    if (slowSave) {
      slowSave = false;
      setTimeout(() => void request.continue(), 700);
      return;
    }
  }
  if (delayReads && url.pathname === '/api/agent-resources/read' && url.searchParams.get('path') === 'notes/example/AGENTS.md') {
    setTimeout(() => void request.continue(), 800);
    return;
  }
...
try {
  ...
  await page.waitForSelector('tbody button[role="combobox"][aria-label="Status for Root Note"]');
  ...
  await click('Source');
  await page.waitForSelector(agentText);
  ...
  const lateRead = page.waitForResponse(r => {
    ...
    return u.pathname === '/api/agent-resources/read' && u.searchParams.get('path') === 'notes/example/AGENTS.md';
  });
  await click('Notebook: Example Guidelines');
  await page.waitForFunction(() => document.body.innerText.includes('Loading document…'));
  await click('Workspace Guidelines');
  await page.waitForFunction(() => document.querySelector('textarea[aria-label="Agent document content"]')?.value.includes('# Workspace Guidelines'));
  ...
  await page.waitForFunction(() => document.querySelector('textarea[aria-label="Agent document content"]')?.value.includes('# Notebook Guidelines'));
  await click('Workspace Guidelines');
  await page.waitForFunction(() => document.querySelector('textarea[aria-label="Agent document content"]')?.value.includes('Unsaved retry'));
  await click('Restore');
  await click('Confirm Restore?');
  await page.waitForFunction(() => document.querySelector('textarea[aria-label="Agent document content"]')?.value === '# Workspace Guidelines\n\nUse **Markdown** notes.\n');
  ...
  await page.waitForFunction(() => !document.querySelector('button[aria-label="Restore"]')?.disabled);
  await page.click('.right-panel-rail [role="tab"][aria-label="Changes"]');
  ...
  await page.screenshot({ path: path.join(product, 'artifacts/qa/shared-agent-editor.png'), fullPage: true });
  console.log('PASS failed save retains its draft and reclick restore uses Git HEAD');
  if (errors.length) throw Error(errors.join('; '));
  console.log('PASS no browser runtime errors');
} catch (error) {
  console.log('Failure', await page.evaluate(() => ({ url: location.pathname, alerts: [...document.querySelectorAll('[role="alert"]')].map(e => e.textContent), text: document.body.innerText.slice(-2000) })));
  throw error;
} finally {
  await browser.close();
