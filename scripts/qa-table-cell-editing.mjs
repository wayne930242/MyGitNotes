...
const contrast=async()=>page.$eval('.live-table-toolbar select',select=>{
 const luminance=color=>{const parts=color.match(/[\d.]+/g).slice(0,3).map(Number).map(n=>{const value=n/255;return value<=0.04045?value/12.92:((value+0.055)/1.055)**2.4;});return parts[0]*0.2126+parts[1]*0.7152+parts[2]*0.0722;};
 return {scheme:getComputedStyle(select).colorScheme,ratios:[...select.options].map(option=>{const style=getComputedStyle(option),a=luminance(style.color),b=luminance(style.backgroundColor);return (Math.max(a,b)+0.05)/(Math.min(a,b)+0.05);})};
});
const rect=selector=>page.$eval(selector,e=>{const r=e.getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,height:r.height};});
...
try {
 ...
 for(const mode of ['expanded','fit']){
   ...
   if(current!==mode){await page.hover(tableRoot);await page.click('[data-table-width-toggle]');}
 ...
 await page.keyboard.type('cancel this change');await page.keyboard.press('Escape');
 if(!await page.$eval(cell,e=>e.textContent.includes('Third line')))throw Error('Escape changed saved multiline text');
 await page.click('.live-md-table tbody tr:nth-child(2) td:nth-child(2)',{count:2});await page.waitForSelector(editor);
 await page.keyboard.type('Meta line one');await page.keyboard.press('Enter');await page.keyboard.type('Meta line two');
