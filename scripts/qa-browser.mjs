...
try {
  ...
  page.on('request', req => {
    ...
    if (u.pathname === '/api/notes') {
      if (req.method() === 'POST') {
        ...
        if (failSave) {
          status = 409;
          body = { error: 'Remote repository changed. Reload the note and retry.' };
        } else {
          remoteNotes = [{ ...remoteNotes[0], ...savedPayload, revision: 'two' }];
          body = { note: remoteNotes[0], commit: { commitHash: 'two' } };
        }
    ...
    if (u.pathname === '/api/auth/agent-token') {
      const requested = JSON.parse(req.postData());
      grants = [{ id: 'grant-id', name: requested.name, write: requested.write, createdAt: Date.now(), source: 'github:owner/repo@main', expiresAt: null }];
      body = { token: demoToken, url: base + '/mcp/' + demoToken, endpoint: base + '/mcp', expiresIn: null };
    }
    if (u.pathname === '/api/auth/agent-tokens/grant-id' && req.method() === 'DELETE') {
      grants = [];
      body = { success: true };
    }
  ...
  await page.waitForSelector('main h3');
