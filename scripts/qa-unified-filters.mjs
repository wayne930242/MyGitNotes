...
try {
 ...
 assert.equal(await page.$('.notes-main .reorder-toggle'),null);
 assert.equal(await page.$('.filter-trigger'),null);
 assert.equal(await page.evaluate(()=>[...document.querySelectorAll('button')].some(b=>b.textContent==='View graph')),false);
 await page.click('[data-tag-filter="blue"]');await count(2);await page.click('[data-tag-filter="red"]');await count(4);
 await page.click('.sidebar-tag-mode button:nth-child(2)');await count(1);
 await page.click('.sidebar-filter-section:last-child summary');
