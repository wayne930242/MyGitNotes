...
try {
 ...
 await page.evaluate(()=>{window.__linePrompt='';window.__linePromptWrites=[];Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async text=>{window.__linePrompt=text;window.__linePromptWrites.push(text);}}});});
 ...
 from=await liveGutterPoint(10);to=await liveGutterPoint(8);await page.mouse.move(from.x,from.y);await page.mouse.down();await page.mouse.move(to.x,to.y,{steps:5});await page.mouse.up();await page.waitForFunction(()=>window.__linePrompt==='Regarding lines 8-10 of `notes/example/root.md`: ');
 ...
 await page.evaluate(()=>{window.__linePromptWrites=[];});
 ...
 await page.mouse.up();await page.waitForFunction(()=>window.__linePrompt==='Regarding lines 8-10 of `notes/example/root.md`: ');
 await page.waitForSelector('[data-line-copy-feedback][data-state="copied"]');
 from=await sourceGutterPoint(10);to=await sourceGutterPoint(8);await page.mouse.move(from.x,from.y);await page.mouse.down();await page.mouse.move(to.x,to.y,{steps:5});await page.mouse.up();await page.waitForFunction(()=>window.__linePrompt==='Regarding lines 8-10 of `notes/example/root.md`: ');
 await page.evaluate(()=>{window.__linePromptWrites=[];});
 ...
 await page.mouse.click(sourceSingle.x,sourceSingle.y);await new Promise(resolve=>setTimeout(resolve,80));await page.mouse.click(sourceSingle.x,sourceSingle.y);
