...
const focusNav = async (label = 'Notes') => {
  const target = await page.$(`.header-nav button[aria-label="${label}"], [data-header-agent][aria-label="${label}"]`);
  await target?.focus();
};
...
try {
  ...
  await page.click('[data-header-command]');
  await page.waitForSelector(palette);
  await page.type(`${palette} input`, 'agent');
  await page.keyboard.press('Enter');
  await page.waitForFunction(selector => !document.querySelector(selector), {}, palette);
  ...
  await page.click('.note-panel-tabs [role="tab"][aria-label="Find in note"]');
  await page.waitForSelector('[role="search"][aria-label="Find in note"] input');
  ...
  await page.click('.note-panel-tabs [role="tab"][aria-label="Find in note"]');
  await page.click('[role="search"][aria-label="Find in note"] input');
  ...
  await page.waitForFunction(() => {
    const input = document.querySelector('textarea[aria-label="Note content"]');
    return input.selectionStart === input.value.indexOf('Second');
  });
  // Document tools reopens the last panel, so Outline becomes the last one before the panel closes.
  await page.click('.note-panel-tabs [role="tab"][aria-label="Outline"]');
  await page.waitForSelector('.note-outline');
  await page.keyboard.press('Escape');
  await page.waitForFunction(() => !document.querySelector('.note-outline'));
  await page.$eval('textarea[aria-label="Note content"]', input => {
