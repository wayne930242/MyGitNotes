...
try {
  ...
  assert(await page.$eval('dialog[open] .study-advanced', details => !details.open), 'Advanced learning settings should be collapsed');
  ...
  assert(await page.$$eval('dialog[open] .study-stage-row input:first-child', inputs => JSON.stringify(inputs.map(input => input.value)) === JSON.stringify(['inbox', 'working', 'done'])), 'Default statuses include archival or require four stages');
  await chooseSelect(page, 'dialog[open] .study-stage-settings .select-trigger', 'last');
  await page.click('dialog[open] .workspace-dialog-actions .ui-button-primary');
  await page.waitForFunction(() => !Object.keys(localStorage).some(key => key.startsWith('github-notes:screen-draft:')));
  assert(readScreen().rows[1].progression.easy === 'last', 'Ordinary lane editor did not save learning settings');
  const beforeStudy = JSON.stringify(readScreen());
  await page.click('#screen-lane-custom .screen-start-study');
  await page.waitForSelector('.screen-study-session');
  assert(JSON.stringify(readScreen()) === beforeStudy, 'Opening study changed stored lane view');
  await page.click('.study-header-actions > button');
  assert(await page.$eval('dialog[open] .study-advanced', details => !details.open), 'Study page settings should also start collapsed');
  await page.click('dialog[open] .study-advanced > summary');
  await page.waitForSelector('dialog[open] .study-stage-settings');
  await chooseSelect(page, 'dialog[open] .study-stage-settings .select-trigger', 'two');
  await page.keyboard.press('Escape');
  assert(JSON.stringify(readScreen()) === beforeStudy, 'Cancel applied study configuration');
  await page.click('.study-header-actions > button');
  assert(await page.$eval('dialog[open] .study-advanced', details => !details.open), 'Study page settings should also start collapsed');
  await page.click('dialog[open] .study-advanced > summary');
  await chooseSelect(page, 'dialog[open] .study-stage-settings .select-trigger', 'last');
  await page.click('dialog[open] .workspace-dialog-actions .ui-button-primary');
  await page.waitForFunction(() => !Object.keys(localStorage).some(key => key.startsWith('github-notes:screen-draft:')));
  assert(readScreen().rows[1].view === 'small' && readScreen().rows[1].progression.easy === 'last', 'Study settings changed layout or did not save');
  await page.click('.study-back');
  await page.waitForSelector('#screen-lane-custom .screen-start-study');
  assert(!await page.$('.study-lane') && readScreen().rows[1].view === 'small', 'Returning did not restore ordinary layout');
  console.log('PASS ordinary lane defaults, independent study entry and settings apply/cancel without view mutation');
  const configPath = path.join(root, '.github-notes.yaml');
  const config = parse(fs.readFileSync(configPath, 'utf8'));
  const unchangedScreen = JSON.stringify(readScreen());
  for (const statuses of [['only'], ['new', 'known'], ['new', 'review', 'known'], []]) {
    ...
    await page.click('dialog[open] .study-advanced > summary');
    const expected = statuses.length ? statuses : ['inbox', 'working', 'done'];
    assert(await page.$$eval('dialog[open] .study-stage-row input:first-child', (inputs, expected) => JSON.stringify(inputs.map(input => input.value)) === JSON.stringify(expected), expected), 'Wrong YAML default stages');
    assert(Boolean(await page.$('dialog[open] .study-single-stage')) === (statuses.length === 1), 'Single-stage explanation is missing or shown for multiple stages');
    await page.keyboard.press('Escape');
  ...
  await page.click('dialog[open] .workspace-dialog-actions .ui-button-primary');
  await page.waitForFunction(() => !Object.keys(localStorage).some(key => key.startsWith('github-notes:screen-draft:')));
  assert(readScreen().rows[1].progression.easy === 'two' && readScreen().rows[1].progression.stages.length === 3, 'Explicit reset did not restore YAML defaults');
  console.log('PASS ordinary edit and study edit share collapsed settings; 1/2/3/omitted YAML statuses preview without writes');
