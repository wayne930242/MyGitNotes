...
const temp = () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'mygitnotes-convert-'));
  dirs.push(dir);
  return dir;
};
...
function forkWorkspace() {
  ...
  up('init', '-b', 'core');
  up('config', 'user.name', 'Core');
  up('config', 'user.email', 'core@example.com');
  write(upstream, { '.github/vercel-sparse-paths.txt': '# list\n.github/vercel-sparse-paths.txt\napps\npackage.json\npnpm-workspace.yaml\nscripts/lib\n', '.github/workflows/deploy-vercel-sparse.yml': 'name: deploy\n', '.gitignore': 'node_modules/\n', 'apps/web/a.ts': 'export {};\n', 'package.json': '{}\n', 'pnpm-workspace.yaml': 'packages: []\n', 'scripts/lib/x.mjs': '', 'scripts/tool.mjs': '', 'docs/CONTEXT.md': '# Core terms\n', 'docs/agent/index.md': '# Product\n', 'README.md': '# Core\n', 'examples/demo.md': '# Demo\n' });
  up('add', '.');
  up('commit', '-m', 'core');
...
it('removes product files from main in one commit and keeps workspace-owned files', () => {
  ...
  expect(git('ls-files').split('\n').sort()).toEqual(['.agents/skills/s/SKILL.md', '.github/workflows/blog-ci.yml', '.gitignore', '.mygitnotes.yaml', 'AGENTS.md', 'blog/index.md', 'docs/CONTEXT.md', 'docs/specs/plan.md', 'notes/a/n.md', 'scripts/tool.mjs']);
...
it('converts a main whose last merged Core predates the product path list', () => {
  ...
  git('checkout', '-q', 'main');
  git('rm', '-q', '.github/vercel-sparse-paths.txt');
  git('commit', '-qm', 'older merge without the list');
  const upstream = git('remote', 'get-url', 'upstream');
  const up = gitIn(upstream);
  up('checkout', '-q', '-b', 'old', 'core~1');
  up('rm', '-q', '.github/vercel-sparse-paths.txt');
  up('commit', '-qm', 'old core without list');
  const old = up('rev-parse', 'HEAD');
  up('checkout', '-q', 'core');
  up('merge', '-q', '-s', 'ours', '-m', 'adopt', old);
  git('fetch', '-q', 'upstream');
  git('merge', '-q', '-s', 'ours', '-m', 'merge old core', old);
  convert(workspace);
