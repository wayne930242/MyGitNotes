...
try{
 await page.setViewport({width:1440,height:1000});await page.goto(base,{waitUntil:'networkidle0',timeout:60000});
 ...
 await page.goto(base+route,{waitUntil:'networkidle0'});await page.waitForSelector('[aria-label="Close note"]');
 assert(!await page.evaluate(()=>document.body.innerText.includes('Path is not a configured note.')));
 await page.reload({waitUntil:'networkidle0'});await page.waitForSelector('[aria-label="Close note"]');
 await page.click('[aria-label="Close note"]');
 console.log('PASS every configured note read, direct note route and editor reload');
 await page.click('[title="Kanban View"]');await page.waitForSelector('[data-status-column="working"]');
 assert(await page.$('[data-status-column="archived"]'));console.log('PASS deployed status defaults and hidden-notes sidebar');
 await page.click('[aria-label="Settings"]');await page.waitForSelector('input[aria-label="MCP client name"]');
 ...
 if(!workspace.capabilities.local){
  ...
  assert.equal(oauth.status,302);const location=new URL(oauth.headers.get('location'));assert.equal(location.hostname,'github.com');assert.equal(location.searchParams.get('redirect_uri'),base+'/api/auth/github/callback');
