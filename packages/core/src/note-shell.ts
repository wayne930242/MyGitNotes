...
export async function callNoteShell(reader: RemoteSource, operation: string, args: Args, write: boolean): Promise<Record<string, unknown>> {
  ...
  if (operation === 'ls') {
    ...
    if (dir === '.') { for (const nb of notebooks) all.set(nb.root, { path: nb.root, type: 'directory' }); }
        ...
        const child = file.path.slice(dir.length + 1).split('/')[0];
        const name = dir + '/' + child;
  ...
  if (operation === 'read') {
    ...
    const lines = textLines(await read(file));
    const startLine = integer(args, 'startLine', 1, 1, Math.max(lines.length + 1, 1));
  ...
  if (operation === 'find') {
    ...
    if (!query || query.length > 1000) throw new SourceError('query must contain 1 to 1000 characters.');
    const matcher = matchNoteGlob(string(args, 'pattern', '**/*'));
    const files = blobs.filter(e => matcher(e.path));
    const fileOffset = integer(args, 'fileOffset', 0, 0, 100000);
    const maxFiles = integer(args, 'maxFiles', 25, 1, 100);
    const maxResults = integer(args, 'maxResults', 100, 1, 500);
    const sensitive = args.caseSensitive === true;
    ...
    for (const file of files.slice(fileOffset, fileOffset + maxFiles)) {
      const lines = textLines(await read(file.path));
      scannedFiles++;
      for (let i = 0; i < lines.length; i++) {
        if ((sensitive ? lines[i] : lines[i].toLowerCase()).includes(needle)) {
  ...
  if (operation === 'mkdir') {
    ...
    const meta: Record<string, unknown> = { ...(args.metadata && typeof args.metadata === 'object' && !Array.isArray(args.metadata) ? (args.metadata as Record<string, unknown>) : {}), title };
  ...
  if (operation === 'update_folder_metadata') {
    ...
    const meta: Record<string, unknown> = { ...currentData, ...(args.metadata && typeof args.metadata === 'object' && !Array.isArray(args.metadata) ? (args.metadata as Record<string, unknown>) : {}) };
