...
const SchedulerSchema = z.object({ due: date, stability: finite, difficulty: finite.max(10), elapsed_days: finite, scheduled_days: finite, learning_steps: finite.int(), reps: finite.int(), lapses: finite.int(), state: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3)]), last_review: date.optional() }).strict();
export const StudyPolicySchema = z.object({ retention: z.number().min(.7).max(.97), intervals: z.array(z.number().int().min(1).max(3650)).min(1).max(20) }).strict().refine(value => value.intervals.every((days, i) => i === 0 || days >= value.intervals[i - 1]), 'Intervals must be nondecreasing.');
const schedule = { id, enabled: z.boolean(), suspended: z.boolean().default(false), policy: StudyPolicySchema, scheduler: SchedulerSchema };
const CardSchema = z.object({ ...schedule, kind: z.enum(['forward', 'reverse', 'cloze']), front: z.array(id).max(100), back: z.array(id).min(1).max(100), masks: z.array(z.object({ pageId: id, start: finite.int(), end: finite.int(), text: z.string().min(1) }).strict().refine(mask => mask.end > mask.start, 'A mask requires a nonempty range.')).max(100).optional() }).strict();
...
export const StudyWorkspaceSchema = z.object({ version: z.literal(1), notes: z.array(NoteSchema).max(2000), events: z.array(EventSchema).max(50000) }).strict().superRefine((value, ctx) => {
  const ids = new Set<string>(), paths = new Set<string>();
  const unique = (key: string) => {
    if (ids.has(key)) ctx.addIssue({ code: 'custom', message: 'Duplicate study identity.' });
    ids.add(key);
  };
  for (const note of value.notes) {
    unique(note.id);
    const key = JSON.stringify([note.notebookId, note.path]);
    if (paths.has(key)) ctx.addIssue({ code: 'custom', message: 'Duplicate study note.' });
    paths.add(key);
...
export function findStudyNote(workspace: StudyWorkspace, source: StudySource): StudyNote | undefined {
  const matches = workspace.notes.filter(note => note.notebookId === source.notebookId && (note.path === source.path || typeof source.metadata.id === 'string' && note.sourceId === source.metadata.id));
...
export function createStudyNote(source: StudySource, now = new Date()): StudyNote {
  ...
  return { id: newId(), notebookId: source.notebookId, path: source.path, ...(typeof source.metadata.id === 'string' ? { sourceId: source.metadata.id } : {}), title: source.title, pages, reading: { step: 0 }, cards: [{ id: newId(), kind: 'forward', enabled: false, suspended: false, front: pages.length > 1 ? [pages[0].id] : [], back: (pages.length > 1 ? pages.slice(1) : pages).map(page => page.id), policy: { retention: .9, intervals: [1, 3, 7, 14, 30] }, scheduler: serializedCard(createEmptyCard(now)) }] };
}
...
export function previewStudyRating(card: StudyCard, rating: Grade, now = new Date()) {
  return serializedCard(fsrs({ request_retention: card.policy.retention, enable_fuzz: false }).next(card.scheduler, now, rating).card);
}
type Action = { kind: 'review'; rating: Grade; } | { kind: 'read' | 'snooze'; due: string; } | { kind: 'fixed' | 'suspend' | 'resume'; } | { kind: 'configure'; policy: StudyPolicy; };
...
export function applyStudyAction(workspace: StudyWorkspace, note: StudyNote, cardId: string, action: Action, now = new Date()): StudyWorkspace {
  ...
  switch (action.kind) {
    case 'review':
      ...
      card.scheduler = previewStudyRating(card, action.rating, now);
      card.enabled = true;
    ...
    case 'fixed': {
      ...
      next.reading = { step: next.reading.step + 1, due: due.toISOString(), lastRead: at };
      break;
    }
    case 'suspend':
      card.enabled = false;
    ...
    case 'resume':
      ...
      break;
    case 'configure':
      card.policy = StudyPolicySchema.parse(action.policy);
      break;
  }
  const event: StudyEvent = { id: newId(), noteId: note.id, cardId, at, kind: action.kind, before, ...(action.kind === 'review' ? { rating: action.rating, algorithm: 'ts-fsrs@5.4.2' } : {}) };
...
export function undoStudyAction(workspace: StudyWorkspace, now = new Date()): StudyWorkspace {
  ...
  return StudyWorkspaceSchema.parse({ ...workspace, notes: workspace.notes.map(note => note.id !== event.noteId ? note : { ...note, stage: event.before!.stage, lastMovedAt: event.before!.lastMovedAt, reading: event.before!.reading, cards: note.cards.map(card => ({ ...card, ...event.before!.cards.find(value => value.id === card.id) })) }), events: [...workspace.events, { id: newId(), noteId: event.noteId, at: now.toISOString(), kind: 'undo', undoOf: event.id }] });
}
...
export function rebindStudyNote(workspace: StudyWorkspace, note: StudyNote, source: StudySource, reset: boolean, now = new Date()): StudyWorkspace {
  ...
  fresh.id = note.id;
  fresh.reading = note.reading;
  if (note.lastMovedAt) fresh.lastMovedAt = note.lastMovedAt;
  if (note.stage) fresh.stage = note.stage;
  fresh.cards[0] = { ...fresh.cards[0], id: note.cards[0].id, enabled: note.cards[0].enabled, suspended: note.cards[0].suspended, policy: note.cards[0].policy, scheduler: reset ? fresh.cards[0].scheduler : note.cards[0].scheduler };
  return StudyWorkspaceSchema.parse({ ...workspace, notes: workspace.notes.map(value => value.id === note.id ? fresh : value), events: [...workspace.events, { id: newId(), noteId: note.id, at: now.toISOString(), kind: 'rebind' }] });
}

export function applyStageAction(workspace: StudyWorkspace, note: StudyNote, laneId: string, status: string | undefined, progression: StudyProgression, action: { kind: 'stage-review' | 'stage-read'; rating: Familiarity; } | { kind: 'stage-postpone'; due: string; }, now = new Date()): StudyWorkspace {
  const stage = action.kind === 'stage-postpone' ? { status: status || progression.stages[0].status, intervalDays: (Date.parse(action.due) - now.getTime()) / 86400000 } : nextStudyStage(progression, status, action.rating);
  ...
  const before = { ...(note.stage ? { stage: note.stage } : {}), ...(note.lastMovedAt ? { lastMovedAt: note.lastMovedAt } : {}), reading: note.reading, cards: note.cards.map(({ id, enabled, suspended, policy, scheduler }) => ({ id, enabled, suspended, policy, scheduler })) };
  const event = { id: newId(), noteId: note.id, cardId: note.cards[0].id, at: now.toISOString(), kind: action.kind, before, transition: { laneId, fromStatus: status ?? null, toStatus: stage.status, intervalDays: stage.intervalDays }, ...(action.kind === 'stage-postpone' ? {} : { rating: action.rating }) };
  return StudyWorkspaceSchema.parse({ ...workspace, notes: [...workspace.notes.filter(value => value.id !== note.id), next], events: [...workspace.events, event] });
}
