...
import { type FolderSnapshot, planFolderChange } from './folder-plan.js';

const snapshot = (): FolderSnapshot => ({ notebooks: [{ id: 'a', title: 'A', root: 'notes/a' }], directories: ['notes/a', 'notes/a/one', 'notes/a/one/sub', 'notes/a/two'], protectedPaths: [], files: new Map([['notes/a/one/_dir.yml', 'title: One\norder: 0\ncustom: keep\n'], ['notes/a/one/note.md', '# Note\n\n[Other](../two/other.md)\n'], ['notes/a/one/sub/_dir.yml', 'title: Child\n'], ['notes/a/two/_dir.yml', 'title: Two\norder: 1\n'], ['notes/a/two/other.md', '# Other\n\n[Note](../one/note.md#part)\n`[Code](../one/note.md)`\n'], ['.github-notes-screen.yaml', 'version: 1\nrows:\n  - id: row\n    kind: custom\n    name: Reading\n    view: small\n    items:\n      - id: pin\n        kind: note\n        notebookId: a\n        path: notes/a/one/note.md\n']]) });
...
it('removes only the folder, moves contents to parent or chosen destination and rejects collisions', () => {
  ...
  collision.files.set('notes/a/note.md', 'Keep me');
...
it('rejects cycles, traversal, notebook roots and protected descendants', () => {
  ...
  const protectedTree = snapshot();
  protectedTree.protectedPaths.push('notes/a/one/.private');
