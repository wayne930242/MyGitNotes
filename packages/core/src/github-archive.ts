...
export async function readGitHubArchive(response: Response, files: { path: string; sha: string; size?: number; }[]): Promise<Map<string, Buffer>> {
  ...
  extract.on('entry', (header, stream, next) => {
    ...
    const file = selected.get(parts.slice(1).join('/'));
    ...
    stream.on('data', (data: unknown) => {
      ...
      if (retained > 24 * 1024 * 1024) {
        extract.destroy(new SourceError('Notebook contents exceed the archive memory limit.', 413));
        return;
      }
      chunks.push(chunk);
    });
  ...
  await pipeline(Readable.fromWeb(response.body as any), bounded(32 * 1024 * 1024), createGunzip(), bounded(128 * 1024 * 1024), extract, { signal: AbortSignal.timeout(30000) });
  return output;
}
