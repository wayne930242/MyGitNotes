...
const fixture: Record<string, string> = {
  ...
  'notes/example/assets/image.png': 'image',
  'notes/example/docs/agent/internal.md': '# Internal',
};
function tree() {
  const entries: any[] = [];
...
let root: string;
beforeEach(() => { root = fs.mkdtempSync(path.join(os.tmpdir(), 'github-notes-source-')); for (const [file, content] of Object.entries(fixture)) { fs.mkdirSync(path.dirname(path.join(root, file)), { recursive: true }); fs.writeFileSync(path.join(root, file), content); } });
...
describe('source configuration', () => {
  it('resolves local paths against the config file, independently of cwd', () => {
    fs.mkdirSync(path.join(root,'runtime')); fs.writeFileSync(path.join(root,'runtime','server.yaml'), 'source:\n  type: local\n  path: ../notes\n');
...
describe('folder and provider parity', () => {
  ...
  it('gives the same note bodies and folder metadata through local and GitHub providers', async () => {
    const request = githubMock(); const remote = new GitHubSource('owner/repo', 'main', undefined, request as typeof fetch);
  ...
  it('rejects stale revisions and duplicate creates before any GitHub write', async () => {
    const request = githubMock(false,true); const remote = new GitHubSource('owner/repo','main','secret',request as typeof fetch);
  ...
  it('reports upstream rate limits and incomplete nonrecursive trees', async () => {
    ...
    const base = githubMock(); const request = vi.fn(async (input: any, init: any) => String(input).includes('/git/trees/') ? new Response(JSON.stringify({ tree: [], truncated: true })) : base(input,init));
  ...
  it('rejects new-file writes through an escaping parent symlink', () => {
    ...
    try { fs.symlinkSync(outside,path.join(root,'escape')); expect(() => resolveSafePath(root,'escape/new/file.md')).toThrow(/Symlink/); }
...
describe('GitHub commit concurrency',()=>{
  ...
  it('recovers a truncated recursive tree by traversing complete subtrees',async()=>{
    const base=githubMock();const all=tree();
    const request=vi.fn(async(input:string,init?:RequestInit)=>{
