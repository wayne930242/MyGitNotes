...
describe('GitLab source configuration', () => {
  it('prefers MyGitNotes environment values while retaining legacy fallbacks', () => {
    expect(loadSourceConfig('/tmp', { MYGITNOTES_SOURCE: 'gitlab', MYGITNOTES_REPOSITORY: source.repository, MYGITNOTES_BRANCH: 'main', MYGITNOTES_GITLAB_URL: site, GITHUB_NOTES_SOURCE: 'github', GITHUB_NOTES_REPOSITORY: 'old/repo', GITHUB_NOTES_BRANCH: 'old', GITHUB_NOTES_GITLAB_URL: 'https://old.test', GITLAB_URL: 'https://fallback.test' })).toEqual(source);
...
describe('GitLab remote contract', () => {
  ...
  it('commits selected notes atomically with per-file version guards and preserves unknown frontmatter', async () => {
    ...
    expect(result.commit.commitHash).toBe(f.head);
    expect(f.writes).toBe(1);
    expect(f.files.get('notes/ex/a.md')).toContain('custom: preserved');
    const body = JSON.parse(String(f.calls.find(c => c.init?.method === 'POST')!.init!.body));
    expect(body).toMatchObject({ branch: 'main', force: false, commit_message: 'docs: update notes' });
  ...
  it('rejects stale and racing edits, symlinks and product writes without partial commits', async () => {
    ...
    f.conflict();
    await expect(r.commitNotes([{ path: 'notes/ex/a.md', content: 'overwrite', metadata: {} }], f.head, 'change')).rejects.toMatchObject({ status: 409 });
    expect(f.writes).toBe(0);
    expect(f.files.get('notes/ex/a.md')).toContain('# Alpha');
  });
  it('supports the same shell write, move and delete operations used by MCP', async () => {
    const f = gitlabFixture();
    await callNoteShell(reader(f), 'write', { path: 'notes/ex/new.md', content: '# New', revision: f.head }, true);
    expect(f.files.get('notes/ex/new.md')).toBe('# New');
    await callNoteShell(reader(f), 'mv', { source: 'notes/ex/new.md', destination: 'notes/ex/moved.md', revision: f.head }, true);
    expect(f.files.has('notes/ex/new.md')).toBe(false);
    expect(f.files.get('notes/ex/moved.md')).toBe('# New');
    await callNoteShell(reader(f), 'rm', { paths: ['notes/ex/moved.md'], revision: f.head }, true);
    expect(f.files.has('notes/ex/moved.md')).toBe(false);
