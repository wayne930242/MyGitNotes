...
function remote(token: string | undefined = 'fixture', push = true, branch = 'main', symlink = false) {
  const request = vi.fn(async (url: string, init?: RequestInit) => {
    ...
    else if (endpoint === '/git/trees/tree?recursive=1') value = { truncated: false, tree: [
      ...Object.keys(files).map(file => ({ path: file, sha: file, type: 'blob', mode: '100644' })),
      ...(symlink ? [{path:'.codex/agents',sha:'link',type:'blob',mode:'120000'}] : []),
    ] };
