...
async function archive(entries: { name: string; content?: string; type?: 'symlink'; linkname?: string; }[]) {
  ...
  const bytes = new Promise<Buffer>((resolve, reject) => {
    pack.on('data', part => parts.push(part));
    pack.on('end', () => resolve(gzipSync(Buffer.concat(parts))));
...
it('accepts only canonical regular-file bytes from the pinned tree', async () => {
  const response = await archive([{ name: 'root/notes/good.md', content: 'exact\r\n' }, { name: 'root/notes/substituted.md', content: 'export-subst changed' }, { name: 'root/notes/link.md', type: 'symlink', linkname: '/etc/passwd' }, { name: 'root/../notes/traversal.md', content: 'bad' }]);
  const selected = [{ path: 'notes/good.md', sha: sha('exact\r\n') }, { path: 'notes/substituted.md', sha: sha('original') }, { path: 'notes/link.md', sha: sha('') }, { path: '../notes/traversal.md', sha: sha('bad') }];
...
it('rejects an oversized archive with 413 instead of emitting an unhandled stream error', async () => {
  ...
  pack.finalize();
  await done;
  const body = gzipSync(Buffer.concat(parts), { level: 0 });
  let offset = 0;
