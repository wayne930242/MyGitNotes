...
export function gitlabFixture(site = 'https://gitlab.example.test/gitlab') {
  ...
  const files = new Map<string, string>([
    ['notes/.github-notes.yaml', manifest], ['notes/ex/a.md', '---\ncustom: preserved\ntags: [work]\n---\n# Alpha\n'],
    ['notes/ex/folder/_dir.yml', 'title: Folder\n'], ['notes/ex/folder/b.md', '# Beta\n'], ['notes/ex/assets/picture.png', 'image bytes'],
  ...
  const request = async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (endpoint.startsWith('/repository/tree?')) {
      ...
      for (const [path, content] of files) {
        const id = sha(content); blobs.set(id, content); entries.push({ path, id, type: 'blob', mode: path.endsWith('symlink.md') ? '120000' : '100644' });
  ...
  return { files, calls, request:request as typeof fetch, get head(){return head;}, get writes(){return writes;},
    setHead(value:string){head=value;}, public(){privateRepo=false;}, readOnly(){canPush=false;}, conflict(){rejectCommit=true;} };
}
