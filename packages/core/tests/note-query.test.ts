...
const note = (overrides: Partial<NoteItem> = {}): NoteItem => ({ id: 'notes/work/alpha.md', path: 'notes/work/alpha.md', notebookId: 'work', title: 'Alpha', tags: ['a'], metadata: {}, content: 'body text', ...overrides });
...
describe('note query matching', () => {
  it('agrees with filterNotes on the shared filters', () => {
    const notes = [note(), note({ path: 'notes/work/deep/beta.md', title: 'Beta', tags: ['b'], status: 'done', content: 'other' }), note({ path: 'notes/work/hidden.md', title: 'Hidden', metadata: { hiden: true } }), note({ path: 'notes/life/gamma.md', notebookId: 'life', title: 'Gamma', tags: [], content: 'body text' })];
    for (const filters of [query(), query({ notebookId: 'work' }), query({ notebookId: 'work', folders: ['notes/work'], descendants: false }), query({ tags: ['a', 'b'], tagMode: 'any' }), query({ status: 'done' }), query({ showHidden: true }), query({ q: 'body' })]) {
      expect(notes.filter(item => noteMatchesQuery(item, filters)).map(item => item.path)).toEqual(filterNotes(notes, filters).map(item => item.path));
    }
  });
...
describe('note query serialization', () => {
  it('omits defaults and repeats list filters', () => {
    ...
    const params = noteQuerySearch({ notebookId: 'work', folders: ['notes/work', 'notes/work/deep'], descendants: false, tags: ['a', 'b'], tagMode: 'all', status: 'inbox', showHidden: true, q: 'text', match: 'title', exclude: ['notes/work/index.md'], sort: 'title', order: 'asc' }, { revision: 'c'.repeat(40), cursor: 'abc', limit: 20, content: true, select: 'paths' });
