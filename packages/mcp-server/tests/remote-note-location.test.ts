...
const reader = (f: ReturnType<typeof gitlabFixture>) =>
  new GitLabSource('https://gitlab.example.test/gitlab', 'group/subgroup/project', 'main', 'fixture-token', f.request);
...
it('reports the path and web page of every note it creates or updates', async () => {
  ...
  expect(await callRemoteTool(reader(f), 'append', { path: 'notes/ex/a.md', content: 'more\n', revision: f.head }, true, app))
    .toMatchObject({ path: 'notes/ex/a.md', url: 'https://notes.example.test/notebooks/ex/notes/a.md' });
  expect(await callRemoteTool(reader(f), 'edit', { path: 'notes/ex/a.md', startLine: 1, endLine: 0, content: 'top\n', revision: f.head }, true, app))
    .toMatchObject({ path: 'notes/ex/a.md', url: 'https://notes.example.test/notebooks/ex/notes/a.md' });
  expect(await callRemoteTool(reader(f), 'save_note', { path: 'notes/ex/folder/c.md', content: '# C', revision: f.head }, true, app))
    .toMatchObject({ path: 'notes/ex/folder/c.md', url: 'https://notes.example.test/notebooks/ex/notes/folder/c.md' });
  expect(await callRemoteTool(reader(f), 'update_note_metadata', { path: 'notes/ex/folder/b.md', status: 'done' }, true, app))
    .toMatchObject({ path: 'notes/ex/folder/b.md', url: 'https://notes.example.test/notebooks/ex/notes/folder/b.md' });
});
