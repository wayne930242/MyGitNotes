...
`,
  ...
  it('rejects user note mutation on the core branch', async () => {
    ...
    await expect(handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/note.md', content: '# Test Note' })).rejects.toThrow(/User content modifications are restricted to workspace branch/);
  });
  ...
  it('rejects path traversal attempts', async () => {
    ...
    await expect(handleSaveNote({ repoRoot: testRepo }, { path: '../outside.md', content: '# Escaped Content' })).rejects.toThrow(/escapes repository root/);
  });
...
`;
    ...
    const saveResult = await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/my-note.md', content: 'This is my first note content.', metadata: { id: 'my-note', title: 'My First Note', status: 'todo', tags: ['first', 'test'], custom_extra_key: 'preserved' } });
    ...
    // Verify read back
    const readResult = await handleReadNote({ repoRoot: testRepo }, { path: 'notes/example/my-note.md' });
...
`;
    ...
    const first = await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/timestamped.md', content: 'First version.', metadata: { title: 'Timestamped' } });
    ...
    const second = await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/timestamped.md', content: 'Second version.', metadata: first.note.metadata });
  ...
  it('rejects asset mutation on the core branch', async () => {
    // Current branch is 'core'
    await expect(handleAddAsset({ repoRoot: testRepo }, { notebookId: 'example', filename: 'test.png', base64Content: Buffer.from('dummy').toString('base64') })).rejects.toThrow(/User content modifications are restricted to workspace branch/);

    await expect(handleDeleteAsset({ repoRoot: testRepo }, { path: 'notes/example/assets/test.png' })).rejects.toThrow(/User content modifications are restricted to workspace branch/);
  });
...
`;
    ...
    const addResult = await handleAddAsset({ repoRoot: testRepo }, { notebookId: 'example', filename: 'photo.png', directory: 'images/sub', base64Content: dummyData });
    ...
    const nonAssetDelete = await handleDeleteAsset({ repoRoot: testRepo }, { path: 'notes/example/note.md' });
    expect(nonAssetDelete.error).toMatch(/not a workspace asset/);

    // 3. Delete the asset
    const deleteResult = await handleDeleteAsset({ repoRoot: testRepo }, { path: 'notes/example/assets/images/sub/photo.png' });
...
`;
    ...
    // Create test notes
    await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/user-guide.md', content: '# Guide\nContact: support@example.com for help.\nRef: TICKET-1234 on 2026-09-12.', metadata: { title: 'User Guide', status: 'inbox' } });

    // 1. Literal search
    const litSearch = await handleSearchNotes({ repoRoot: testRepo }, { query: 'support@example.com' });
    ...
    // 2. Regex search
    const regexSearch = await handleSearchNotes({ repoRoot: testRepo }, { query: 'TICKET-\\d+', isRegex: true });
...
`;
    ...
    await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/todo.md', content: '# Tasks\nFix ISSUE-101 and ISSUE-102.', metadata: { title: 'Todo', status: 'working' } });

    // 1. Dry run
    const dryRunRes = await handleReplaceNotes({ repoRoot: testRepo }, { find: 'ISSUE-(\\d+)', replace: 'BUG-$1', isRegex: true, dryRun: true });
    ...
    // 2. Real replacement
    const liveRes = await handleReplaceNotes({ repoRoot: testRepo }, { find: 'ISSUE-(\\d+)', replace: 'BUG-$1', isRegex: true });
...
`;
    ...
    await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/feature.md', content: '# Feature Planning\nDetailed specification...', metadata: { title: 'Feature Planning', status: 'inbox', tags: ['v1'] } });
    ...
    const updateRes = await handleUpdateNoteMetadata({ repoRoot: testRepo }, { path: 'notes/example/feature.md', status: 'done', tags: ['v1', 'shipped'] });
  ...
  it('supports mkdir with metadata, get_folder_metadata, and update_folder_metadata with atomic commits', async () => {
    // Current branch is 'core'
    await expect(handleMkdir({ repoRoot: testRepo }, { path: 'notes/example/projects', title: 'Projects', order: 1 })).rejects.toThrow(/User content modifications are restricted to workspace branch/);
...
`;
    ...
    const mkdirRes = await handleMkdir({ repoRoot: testRepo }, { path: 'notes/example/projects/backend', title: 'Backend Services', order: 2, description: 'Microservices and backend APIs' });
    ...
    const dupRes = await handleMkdir({ repoRoot: testRepo }, { path: 'notes/example/projects/backend', title: 'Backend Services' });
    expect(dupRes.error).toMatch(/already exists/);

    // 2. get_folder_metadata
    const getRes = await handleGetFolderMetadata({ repoRoot: testRepo }, { path: 'notes/example/projects/backend' });
    ...
    const updateRes = await handleUpdateFolderMetadata({ repoRoot: testRepo }, { path: 'notes/example/projects/backend', order: 10, description: 'Updated backend description' });
    expect(updateRes.success).toBe(true);
  ...
  it('exposes exactly 18 consolidated tools and absorbs redundant endpoints', async () => {
    ...
    // 18 primary tools
    const expected = ['list_folders', 'get_workspace_config', 'list_notebooks', 'list_notes', 'read_note', 'save_note', 'delete_note', 'read_agent_resource', 'list_assets', 'add_asset', 'delete_asset', 'get_git_status', 'git_commit', 'update_core', 'search_notes', 'replace_notes', 'get_statuses', 'mkdir'];
    expect(toolNames.sort()).toEqual(expected.sort());

    // Redundant absorbed endpoints are not in the exposed list
    const absorbed = ['get_note_metadata', 'update_note_metadata', 'get_folder_metadata', 'update_folder_metadata', 'list_agent_resources', 'check_core_update'];
...
`;
    ...
    await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/sample.md', content: '# Sample Note Body\nSome text.', metadata: { title: 'Sample Note', status: 'inbox', tags: ['alpha'] } });

    // 2. read_note with metadataOnly: true
    const metaOnly = (await handleReadNote({ repoRoot: testRepo }, { path: 'notes/example/sample.md', metadataOnly: true })) as any;
    ...
    const updateRes = (await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/sample.md', status: 'working', tags: ['alpha', 'beta'], title: 'Updated Sample Title' })) as any;
    expect(updateRes.success).toBe(true);
    ...
    const fullNote = (await handleReadNote({ repoRoot: testRepo }, { path: 'notes/example/sample.md' })) as any;
    expect(fullNote.note.content.trim()).toBe('# Sample Note Body\nSome text.');

    // 4. list_folders with path
    await handleMkdir({ repoRoot: testRepo }, { path: 'notes/example/docs', title: 'Documentation', order: 1 });
    const singleFolder = (await handleListFolders({ repoRoot: testRepo }, { path: 'notes/example/docs' })) as any;
...
`;
    ...
    // Save a note
    await handleSaveNote({ repoRoot: testRepo }, { path: 'notes/example/test.md', content: '# Legacy Test', metadata: { title: 'Legacy Note', status: 'inbox' } });

    const server = createMCPServer(testRepo);
    // Legacy call: get_note_metadata
    const getMetaReq = { method: 'tools/call', params: { name: 'get_note_metadata', arguments: { path: 'notes/example/test.md' } } };
    ...
    const listResReq = { method: 'tools/call', params: { name: 'list_agent_resources', arguments: {} } };
