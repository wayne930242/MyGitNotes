...
export async function handleAddAsset(
  ctx: ToolContext,
  args: { notebookId: string; filename: string; base64Content: string }
) {
  await assertUserWorkspaceBranch(ctx.repoRoot);
  const config = loadWorkspaceConfig(ctx.repoRoot);
  if (!config) return { error: 'Workspace not configured' };
  ...
  const relPath = path.relative(ctx.repoRoot, targetPath).replace(/\\/g, '/');
  ...
  return {
    ...
    path: relPath,
    markdownRef: `![${safeFilename}](${nb.assets || 'assets'}/${safeFilename})`,
    commit,
  };
}
