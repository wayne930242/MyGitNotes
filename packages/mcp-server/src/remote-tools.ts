...
const filePathField = str('Repository-relative path of a note inside a configured notebook, or of a skill file (<dir>/.agents/skills/<name>/...) from list_skills or invoke_skill.', { minLength: 1, maxLength: 1024 });
const hintField = str('Present when the note has an agent system: call get_system_prompt and list_skills before creating or editing notes there.');
const targetFields = { notebookId: str('Notebook ID; targets the notebook root.'), path: str('Note or folder path inside a notebook; a note path works before the note exists. Pass notebookId or path, not both.', { minLength: 1, maxLength: 1024 }) };
const skillSummary = { name: str('Skill name, equal to its directory name'), description: str('Frontmatter description'), path: str('Path of the SKILL.md file'), directory: str('Skill directory') };
...
const noteSummarySchema = object({ id: str('Note identifier'), path: pathField, notebookId: str('Notebook ID'), title: str('Title'), description: str(`Frontmatter description, else the opening body line, at most ${NOTE_DESCRIPTION_LIMIT} characters`), status: { type: ['string', 'null'], description: 'Note status' }, tags: array(str('Tag')), metadata, size: int('Bytes of the file', 0, Number.MAX_SAFE_INTEGER) }, ['path', 'title', 'notebookId', 'description', 'tags', 'metadata'], true);
...
export async function callRemoteTool(reader: RemoteSource, name: string, args: Record<string, unknown>, write: boolean, appUrl?: string): Promise<Record<string, unknown>> {
  ...
  const notebook = skillFile(file, notebooks) ? undefined : notebooks.find((nb) => file.startsWith(`${nb.root}/`));
...
async function runRemoteTool(reader: RemoteSource, name: string, args: Record<string, unknown>, write: boolean): Promise<Record<string, unknown>> {
  ...
  switch (name) {
    ...
    case 'list_notes': {
      ...
      // Paths come from the snapshot tree, so only the listed page is ever read.
      const paths: string[] = [];
      for (const nb of notebooks) paths.push(...(await reader.notePaths(nb)).sort((a, b) => a.localeCompare(b)));
      const listed = paths.slice(offset, offset + limit);
      await reader.prefetchFiles(listed);
      const read: NoteItem[] = [];
      for (let i = 0; i < listed.length; i += 6) read.push(...await Promise.all(listed.slice(i, i + 6).map((file) => reader.note(file))));
    ...
    case 'list_skills':
    case 'invoke_skill':
      return callAgentSystem(reader, name, args);
    default:
