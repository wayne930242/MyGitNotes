...
function tool(name: string, description: string, properties: Schema, required: string[], output: Schema): Tool {
  ...
  return {
    name,
    description,
    inputSchema: object(properties, required) as Tool['inputSchema'],
    outputSchema: output as Tool['outputSchema'],
    annotations: {
      title: name,
...
export async function callRemoteTool(
  ...
  switch (name) {
    ...
    case 'list_assets':
      return { assets: await reader.assets(args.notebookId as string | undefined) };
    case 'add_asset': {
      const snapshot = await reader.getSnapshot();
      const rev = String(args.revision || snapshot.sha);
      ...
      return {
        ...
        path: dest,
        markdownRef: `![${path.posix.basename(dest)}](${markdownRel})`,
      };
    }
    case 'delete_asset': {
      const snapshot = await reader.getSnapshot();
      ...
      return { success: true, path: String(args.path), commit: res.commit };
    }
    case 'get_statuses': {
      const config = await reader.config();
    ...
    case 'replace_notes': {
      ...
      if (dryRun) {
        return {
          ...
          search,
          replace,
          matchedFiles: modified.map((n) => n.path),
          totalFiles: modified.length,
        };
      }
      ...
      const receipt = await reader.commitChanges(
        ...
        'replace',
        'notes',
        `docs(notes): replace "${search}" across ${modified.length} notes`
      );
      return {
        success: true,
        dryRun: false,
