...
在筆記本根目錄或任意子資料夾放入 `index.md` 或 `README.md`，該資料夾的列表、卡片及看板便會在資料夾卡片列最前面顯示白色「索引」卡片。兩者都有時以 `index.md` 為主，`README.md` 保留為一般筆記。卡片使用文件圖示，點擊後沿用一般筆記的開啟及編輯方式，被選作索引的檔案不再重複出現在下方筆記中。選定索引後才套用隱藏設定，因此 `index.md` 被隱藏時不會改用 `README.md`。搜尋、狀態／標籤篩選及「攤開」時不顯示索引卡片。
