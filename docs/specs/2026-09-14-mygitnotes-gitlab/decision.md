...
核心規則已確認：[產品指引](../../agent/product/index.md)。產品與使用者工作區資料分離。更名涵蓋產品標題、操作提示、文件與架構圖；正式 repo 使用 `wayne930242/MyGitNotes`，部署 URL 與已發布套件識別沿用既有值。

- 使用者核准測試部署共用既有免費 Upstash Redis。新增可選資料前綴以隔離 sessions、credentials、grants 與 refresh locks；既有部署未設定時沿用原有鍵值。

## Repo 與環境變數更名

使用者已將 repo 更名並提供 `git@github.com:wayne930242/MyGitNotes.git`，要求直接更新設定。產品 origin、工作區 upstream、正式連結及受影響部署使用新路徑。產品設定統一使用 MYGITNOTES_*，程式保留 GITHUB_NOTES_* fallback；兩者並存時新名稱優先。平台 OAuth 的 GITHUB_CLIENT_* 與 GITLAB_CLIENT_* 保留。
