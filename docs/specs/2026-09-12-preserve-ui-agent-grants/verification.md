...
The first hosted Actions run exposed two clean-checkout assumptions hidden by the existing developer workspace: tests import built workspace package exports, and an old invariant test required private root notes on Core. CI now builds before testing; the invariant test checks the canonical examples and resource ownership without depending on local user files. The failed run stopped before modifying main.
