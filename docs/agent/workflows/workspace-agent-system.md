...
每個工作區的 `main` 分支自行追蹤根目錄 `AGENTS.md`、`.agents/**` 與 `.codex/**`。Core 不追蹤這些實體檔案，也不透過共用 `.gitignore` 隱藏它們。產品開發指引位於 [product/index.md](../product/index.md)，產品技能位於該目錄的 `skills/`。
...
Git 路徑所有權涵蓋完整 Agent 目錄，介面可存取範圍則限於上述文件。`.codex/auth.json`、`.codex/config.toml`、執行紀錄、隱藏檔與腳本不會開放；文件不得透過符號連結讀寫其他檔案。憑證與執行紀錄應維持本機私有，不提交。
