...
在筆記本根目錄或任何子資料夾放入一份小寫的 `index.md`，就能替該資料夾加上索引卡片。它使用資料夾卡片的版型、白色背景與文件圖示，排列在其他資料夾之前。
...
每份 `index.md` 只對應所在的資料夾。沒有索引檔的資料夾，就不會出現索引卡片；不會借用上層的索引。
...
- 即使資料夾只有 `index.md`，仍會顯示索引卡片。
- 被隱藏的索引不顯示卡片；打開「顯示隱藏筆記」才會出現。詳見[狀態與可見性](getting-started/status-and-visibility.md)。
